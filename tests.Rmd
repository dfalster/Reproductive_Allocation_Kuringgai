...
```{r}
#pdf("figs2/weight_diameter_segments_2.pdf", height=11.3, width=8)
#plot
par(mfcol=c(2,2), cex=1, omi=c(.1,.1,.1,.1), mai=c(1,1,.1,0.1)) 
data <- subset(HarvestData)
data <- split(data, data$species)

for(spp in names(data)) {
plot(stem_weight~diameter,data[[spp]],log="xy",cex=.7, main=spp,col=col.age(data[[spp]]$age),pch=16,ylab="stem (circle) + leaf (tri)"
)
points(leaf_weight~diameter,data[[spp]],cex=.7,pch=17,col=col.age(data[[spp]]$age))
}
#dev.off()
...
```{r, echo=FALSE,message=FALSE,warning=FALSE, fig.width=20, fig.height=20}
#pdf("figs2/tradeoff.pdf", height=11.3, width=8)
#plot
...
plot(leaf_turnover~age,SummaryInd,log="xy", col=col.spp(species),pch=16)
SummaryInd$leaf_net <- SummaryInd$growth_leaf + SummaryInd$leaf_turnover
plot(leaf_net~age,SummaryInd,log="xy", col=col.spp(species),pch=16)
plot(leaf_turnover~leaf_net,SummaryInd,log="xy", col=col.age(age),pch=16)
abline(0,1)

```{r, echo=FALSE,message=FALSE,warning=FALSE, fig.width=20, fig.height=20}
par(mfcol=c(4,4), cex=1, omi=c(.1,.1,.1,.1), mai=c(1,1,.3,0.1)) 
yvar <- "leaf_turnover"
xvar <- "leaf_net"
...
for(spp in names(data)) {
  plot_yvar_vs_xvar(data[[spp]], yvar, xvar,log="xy",cex=1.4, xlab="",ylab="",main=spp,col=col.age(data[[spp]]$age))
  abline(0,1)
  abline(log10(.111),1)
  abline(log10(.010101),1)
}
