...
```{r, echo=FALSE,message=FALSE}
plot((prop_max_repro+.1)~(prop_maxH), SummaryInd, log="y",col=col.spp[as.factor(species)],pch=16,xlim=c(4e-03,1))
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
...
For these calculations, height is scaled relative to max height. This should be a way to determine RSOM - RSOM would be the prop_maxH at which Reproductive Investment begins.
...
These plots show very clearly that for all species RGR suddenly dips when RA becomes non-zero. Plotting against "total_weight" or "GrowthInvestment" is much less clear - in other words, an individual's height is the best way to sort them into plants investing more in growth vs reproduction. There are of course some non-reproductive plants with "prop_maxH" above the obvious cut-off, but these are almost certainly "reproductively mature" plants not reproducing in a given year.  
...
results$r.squared <- round(results$r.squared,digits=3)
results$p.value <- round(results$p.value,digits=3)
names(results) <- c("species","r.sq_total","p-total","n","intercept","coef_RA","coef_prop_maxH","coef_interaction","SS_RA","SS_prop_maxH","SS_interaction")
results
...
names(results2) <- c("species","r.sq.prop_maxH","p.prop_maxH","n","a","b")
results2
