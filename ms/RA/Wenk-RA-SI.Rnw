...
\item For a each part in turn, we try to identify the most obvious predecessor, as the one with minimal amount of progression needed to satisfy successive censuses. The predecessor is removed from the list of possible predecessors and the process is repeated until all parts have predecessors.
