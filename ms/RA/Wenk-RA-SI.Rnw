...
\title{Investment in reproduction by 14 iteroparous perennials and its coordination with other life-history and functional traits}
...
flowering_parts <- read.csv("../Accessory/Sup_Table_flowering_parts.csv", stringsAsFactors=FALSE, check.names=FALSE)
...
\caption{Relationship between a species leaf mass per area and it's rate of annual leaf turnover.}
\label{fig:leaf_loss}
\end{figure}
\clearpage
...
\item For a each part in turn, we try to identify the most obvious predecessor, being the one with minimal amount of progression needed to satisfy successive censuses. The predecessor is removed from the list of possible predecessors and the process is repeated until all parts have predecessors.
