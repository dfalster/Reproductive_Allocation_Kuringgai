...
Also shown below  in Figs. \ref{fig:growth_Banksia_ericifolia}, \ref{fig:growth_Boronia_ledifolia},...., \ref{fig:growth_Pultenaea_tuberculata} are the relationships used to estimate growth in leaf and stem mass for each species.
...
estimated growth trajectory for each individual across an entire year.}
