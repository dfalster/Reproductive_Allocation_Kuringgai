...
\label{tab:propagule_invest_vs_repro}
\begin{tabular}{p{4cm}|x{.94cm}x{3cm}p{1.5cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
...
\label{tab:propagule_invest_vs_repro}
\begin{tabular}{p{4cm}|x{.94cm}x{1cm}p{1.5cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
...
\subsection{investment categories by species}
...
accessory\_parts.yml
flowerCategories.csv and flowerCategories\_meta.csv
flowerParts.csv and flowerParts\_meta.csv
individuals.csv and individuals\_meta.csv
MultiplierTable.csv
reproduction.csv and reproduction\_meta.csv
seedMultiplier.csv

The following files contain supplmentary data about the species and sites:
seedsize.csv and seedsize\_meta.csv
sites.csv and sites\_meta.csv
species.csv
...
As the reproductive cycle progresses, tissue parts were repeatedly split into two (or more) parts. For instance, a bud, once open, divides into being petals + stamens, a sepal, and a stigma. As a pollinated stigma develops into a fruit, it will be further divided into seed, seed pod, and potentially additional parts. Along the developmental trajectory, various reproductive tissues are designated as reaching their final development stage, the point in the reproductive cycle where they no longer increase in mass. For some tissues, such as petals, final development is reached long before a mature seed forms, while other tissues, such as the seed and seedpod do not reach final development until the end of the developmental trajectory. 

Each species reproductive tissue development initiates with a bud or inflorescence bud, which increases in size and is divided into multiple tissue types several times during development. The central development axis terminates with the formation of a mature seed, while side branches end in fully developed tissues (such as petals, calyx, pedicel) and aborted tissues (aborted seeds, aborted fruit) at the point where they cease to increase in mass. The developmental flowchart for each species is shown in the maps in the next section.
...
\subsection{R scripts used to calculate reproductive investment}
