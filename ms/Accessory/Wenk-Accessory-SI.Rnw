...
make_table_figure3_main_text <- function(SummarySpp) {
  ...
  for (i in c("seedset", "embryo_endo_costs", "prop_pollen_attract_vs_success", 
                               "prop_prepollen_success", "prop_postpollen_success")) {
    acc_costs_spp[[i]] <- round(acc_costs_spp[[i]], digits = 3)
...
\begin{table}[h]
\centering
\caption{NEED TITLE - showing values in Figure Table 3 in the main text}
...
\begin{table}[h]
\centering
\caption{Scaling of reproductive tissue costs with total seed size. In this manuscript all "seed size" calculations use embryo and endosperm size, not propagule size. Most previous research has used propagule size as a measure of "seed size" and noted isometric relationships between various tissue costs and "seed size". Using embryo and endosperm size, these relationship did not exist for this data set, but are present if "seed size" includes the entire propagule.}
...
make_table_propagule_invest_vs_all_repropro_inv <- function(SummaryInd) {
  ...
  acc_slopes <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="SMA", log="xy")$groupsummary %>%
                select(group,n,r2,pval, Int, Slope)
  names(acc_slopes) <- c("Species","n_propagule","r2_propagule","p_propagule", "int_propagule","slope_propagule")

  yvar <- "repro_inv"
  xvar <- "flower_inv"
  data <- subset(SummaryInd,SummaryInd[[xvar]]>0&SummaryInd[[yvar]]>0)

  acc_slopes2 <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="SMA",  log="xy")$groupsummary %>%
                select(group,r2,pval, Int, Slope)
  names(acc_slopes2) <- c("Species","r2_flower","p_flower", "int_flower","slope_flower")
  
  acc_slopes <- dplyr::left_join(acc_slopes, acc_slopes2, by ="Species")
  names(acc_slopes) <- c("Species","n","r$^2$","p","int.", "slope","r$^2$","p","int.", "slope")
...
\begin{tabular}{p{4cm}|x{.75cm}|x{1cm}x{1.5cm}x{1cm}x{1cm}|x{1cm}x{1.5cm}x{1cm}p{1cm}}
\hline
& & \multicolumn{4}{c|}{Total reproductive investment} & \multicolumn{4}{c}{Total reproductive investment} \\
& & \multicolumn{4}{c|}{vs. propagule investment} & \multicolumn{4}{c}{vs. flower investment} \\
<<echo=FALSE, results='asis', warning=FALSE>>=
