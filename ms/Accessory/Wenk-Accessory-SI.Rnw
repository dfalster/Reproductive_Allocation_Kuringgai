...
library(xtable)
library(knitr)
opts_chunk$set(echo=FALSE, fig.pos = "!h", fig.align="center", comment = NA, warning = FALSE)
...
make_table_figure3_main_text <- function(SummarySpp) {
  acc_costs_spp <- dplyr::select(SummarySpp[["mean"]], species, 
                               seedset, embryo_endo_costs, prop_pollen_attract_vs_success, 
                               prop_prepollen_success, prop_postpollen_success)

  acc_costs_spp$species <-  paste("\\textit{",labels.spp.full(),"}", sep="")
  for (i in c("seedset", "embryo_endo_costs", "prop_pollen_attract_vs_success", 
                               "prop_prepollen_success", "prop_postpollen_success")) {
    acc_costs_spp[[i]] <- round(acc_costs_spp[[i]], digits = 4)
  
  }

  names(acc_costs_spp) <- c("Species","Ratio seeds to ovules","seed size",
                                  "Prop. of successful inv. to pollen attraction tissues",
                                  "Prop. of pollen attraction tissues to successful inv.",
                                  "Prop. of provisioning tissues to successful inv.")

  acc_costs_spp
}

TableS1 <- make_table_figure3_main_text(SummarySpp)
@

\begin{table}[h]
\centering
\caption{Values plotted in Figure 3 in the main text, showing the linkage across species between maternal optimism and seed size.}
\label{tab:fig_3_table}
\begin{tabular}{p{4cm}p{2cm}p{2cm}p{2cm}p{2cm}p{2cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
print(xtable(TableS1)
...
<<echo=FALSE, results='asis', warning=FALSE>>=
make_table_Table2_using_seed <- function() {
  
  f <- function(yvar, xvar, data) {
      sma(data[[yvar]] ~ data[[xvar]],method="SMA", log="xy")$groupsummary %>% 
      mutate(x = xvar, y=yvar) %>%
      select(x, y, everything()) %>%
      mutate(
       int =  paste(format_r2(Int)," (",format_r2(Int_lowCI)," - ",format_r2(Int_highCI),")",sep=""),
       slope = paste(format_r2(Slope)," (",format_r2(Slope_lowCI)," - ",format_r2(Slope_highCI),")",sep=""),
       r2 = format_r2(r2),
       pval = format_p(pval),
       slope_test = format_p(Slope_test_p)
    )
  }

  dplyr::bind_rows(
    f("repro_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("accessory_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("success_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("pollen_attract_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("provisioning_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("discarded_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("prepollen_discarded_costs", "embryo_endo_costs", SummarySpp[["mean"]]),
    f("postpollen_aborted_costs", "embryo_endo_costs", SummarySpp[["mean"]])
  ) %>%
  mutate(
    variable = c("total reproductive","total accessory","seed","pollen attraction","provisioning", "failed tissue","failed pre-pollination","failed provisioning")) %>%
  select(variable, n, r2, int, slope)
}
TableS19 <- make_table_Table2_using_seed()
names(TableS19) <- c("Costs","n","r$^2$","Int (95$\\%$ CI)", "Slope (95$\\%$ CI)")
...
\centering
\caption{
Scaling of reproductive tissue costs with seed size.  Each data point is a species average value. Costs are all calculated as mg carbon investment per mature seed. All variables showed a strong correlation with seed size (p < 0.0001). The table shows properties of SMA line fits, between different variables and seed size. All variables are log-scaled.
}
\label{tab:repro_tissues_vs_prop_size}
\begin{tabular}{p{5cm}|x{.75cm}x{.75cm}x{3.5cm}p{3cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
...
make_table_propagule_invest_vs_all_repropro_inv <- function(SummaryInd) {
  ...
  acc_slopes <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="OLS", log="xy")$groupsummary %>%
        mutate(
          int =  paste(format_r2(Int)," (",format_r2(Int_lowCI)," - ",format_r2(Int_highCI),")",sep=""),
          slope = paste(format_r2(Slope)," (",format_r2(Slope_lowCI)," - ",format_r2(Slope_highCI),")",sep=""),
          r2 = format_r2(r2),
          pval = format_p(pval)) %>%
      select(group,n,r2, int, slope)
  names(acc_slopes) <- c("Species","n_propagule","r2_propagule","int_propagule","slope_propagule")

  yvar <- "repro_inv"
  xvar <- "flower_inv"
  data <- subset(SummaryInd,SummaryInd[[xvar]]>0&SummaryInd[[yvar]]>0)

  acc_slopes2 <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="OLS",  log="xy")$groupsummary %>%
        mutate(
          int =  paste(format_r2(Int)," (",format_r2(Int_lowCI)," - ",format_r2(Int_highCI),")",sep=""),
          slope = paste(format_r2(Slope)," (",format_r2(Slope_lowCI)," - ",format_r2(Slope_highCI),")",sep=""),
          r2 = format_r2(r2),
          pval = format_p(pval)) %>%
      select(group,r2, int, slope)
  names(acc_slopes2) <- c("Species","r2_flower","int_flower","slope_flower")
  
  acc_slopes <- dplyr::left_join(acc_slopes, acc_slopes2, by ="Species")
  names(acc_slopes) <- c("Species","n","r$^2$","intercept", "slope","r$^2$","intercept", "slope")
  ...
  acc_slopes$n <- as.character(round(acc_slopes$n, 0))
...
\begin{table}[h]
\centering
\caption{Details on linear models fitted across individuals within each species to predict total reproductive investment from (a) propagule investment and (b) flower investment, where flower investment is defined as 'flower weight $\times$ bud count'. Due to the high proportion of aborted flowers, flower investment was a better proxy of reproductive investment than was propagule investment. This was true both across species (Figure 4, Table 2 in the main text) and within species, as is shown here.}
\label{tab:propagule_invest_vs_repro}
\begin{tabular}{p{2.5cm}|x{.75cm}|x{0.75cm}x{2.75cm}x{2.75cm}|x{0.75cm}x{2.75cm}p{2.75cm}}
\hline
...
i <- data$Previous_names != ""
data$Species_name[i] <- sprintf("%s (previously %s)", data$Species_name[i],data$Previous_names[i])
data <- dplyr::select(data,-Previous_names)

names(data) <-  clean_headers(names(data))

print(xtable(data,
          align= 'p{1cm}p{5cm}p{3cm}p{5cm}p{2cm}',
          hline.after=c(1),
          label= "tab:species",
...
print(xtable(data,
          align= 'p{0.1cm}p{2cm}p{1cm}p{1cm}p{1cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.2cm}p{5cm}',
          hline.after=c(1),
          digits=c(0,0,0,2,0,0,0,4,4,0),
          label= "tab:sites",
