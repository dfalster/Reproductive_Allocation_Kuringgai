...
print_table_metadata <- function(path, align = NULL) {
  ...
  print(xtable(data,
            ...
            label= label,
            caption=
              sprintf("Details on variables within the file \\texttt{%s}.",
              gsub("\\_meta", "", file, fixed = TRUE))),
      , table.placement = "h"
      , caption.placement = "top"
      , include.rownames = FALSE
      , include.colnames = TRUE
...
names(TableS15) <- c("species","seed count scaled to leaf area","ovule count scaled to leaf area","pollen-attraction costs scaled to leaf area","choosiness (1/seedset)","success","discarded","success","discarded","pollen-attraction","provisioning")
...
\begin{table}[H]
\centering
\caption{Species values for data plotted in Figure 2: shifts in reproductive tissue allocation with seed size. Data for reproductive costs (Figure 2, panel a), Seed-size, pollen-attraction costs, and provisioning costs (Figure 2, panel f) are presented in Table 1 in the main text and are not included here.}
\label{tab:species}
\begin{tabular}{p{4cm}|p{1.5cm}p{1cm}|p{1.5cm}p{1cm}|p{1.5cm}p{1cm}|p{1cm}p{1.5cm}|p{1.5cm}p{1.5cm}}
\hline
& \multicolumn{4}{c|}{} & \multicolumn{2}{c|}{Proportion} & \multicolumn{2}{c}{Proportion}& \multicolumn{2}{c}{Proportion} \\
& \multicolumn{4}{c|}{} & \multicolumn{2}{c|}{pollen-attraction} & \multicolumn{2}{c}{provisioning}& \multicolumn{2}{c}{success} \\
& \multicolumn{4}{c|}{} & \multicolumn{2}{c|}{investment to} & \multicolumn{2}{c}{investment to}& \multicolumn{2}{c}{investment to} \\

% \begin{tabular}{p{3cm}|p{1cm}p{1.5cm}p{1cm}|p{1.5cm}p{1cm}|p{1.5cm}p{1cm}|p{1cm}p{1.5cm}|p{1.5cm}p{1.5cm}}
% \hline
% &&&& \multicolumn{2}{c|}{Proportion} & \multicolumn{2}{c}{Proportion}& \multicolumn{2}{c}{Proportion} \\
% &&&& \multicolumn{2}{c|}{pollen-attraction} & \multicolumn{2}{c}{provisioning}& \multicolumn{2}{c}{success} \\
% &&&& \multicolumn{2}{c|}{investment to} & \multicolumn{2}{c}{investment to}& \multicolumn{2}{c}{investment to} \\
...
@
\hline
& \multicolumn{1}{c|}{Figure reference} & \multicolumn{3}{c|}{Fig 2, panel a} & \multicolumn{2}{c|}{Fig 2, panel b} & \multicolumn{2}{c}{Fig 2, panel c} & \multicolumn{2}{c}{Fig 2, panel d} & \multicolumn{2}{c}{Fig 2, panel e} \\

\end{tabular}

{\color{red}
\begin{itemize}
  \item Started reworking table headers but have no idea what they're meant to be.
  \item Decrease decimal places?
  \item Swing page to landscape?
  \item Add units?
\end{itemize}
}
...
\section{Additional details on methods and details about input files}
...
\subsubsection{Banksia ericifolia}
...
\subsubsection{Boronia ledifolia}
...
\subsubsection{Conospermum ericifolium}
...
\subsubsection{Epacris microphylla}
...
\subsubsection{Grevillea buxifolia}
...
\subsubsection{Grevillea speciosa}
...
\subsubsection{Hakea teretifolia}
...
\subsubsection{Hemigenia purpurea}
...
\subsubsection{Leucopogon esquamatus}
...
\subsubsection{Persoonia lanceolata}
...
\subsubsection{Petrophile pulchella}
...
\subsubsection{Phyllota phylicoides}
...
\subsubsection{Pimelea linifolia}
...
\subsubsection{Pultenaea tuberculata}
...
\subsubsection{OrderedListOfParts.R}
...
\subsubsection{FlowerParts.R}

Script to calculate the unit weights of all reproductive tissues for each individual, including special calculations for:
...
\subsubsection{GraphRepresentation.R}

Script describing defining the plant maps for each species. Each plant map begins with a bud (or the appropriate starting point for reproductive tissue development for a specific species) and depicts graphically how the tissue develops through the plant's reproductive cycle to a seed or mature fruit. During the reproductive cycle, there will be branches off the central tissue axis (development from bud to seed). Some of these lead to aborted tissue types (colored red in the graph maps below) and others lead to tissues that reach their final development stage and are then shed by a plant (e.g. petals, calyx, seed pod; colored green in the graph maps below). For some species, there are mutiple axes of development. For instance, cones and flowers on a cone have separate axes, because the number of flowers on a cone is inconsistent across individuals. A few species have aborted plant parts that are listed as a separate axis. In these cases, the aborted unit weighs less than a bud and therefore cannot be shown as ever having reached the stage of a bud.

Uses data files and data frames: PartsSummary (by species), data/MultiplierTable.csv
Output: GraphMaps (by species)


\subsubsection{Investment.R}
...
\subsubsection{ReproductionTissues.R}
