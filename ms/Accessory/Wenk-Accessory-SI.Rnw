...
accessory_parts.yml
flowerCategories.csv and flowerCategories_meta.csv
flowerParts.csv and flowerParts_meta.csv
individuals.csv and individuals_meta.csv
MultiplierTable.csv
reproduction.csv and reproduction_meta.csv
seedMultiplier.csv

The following files contain supplmentary data about the species and sites:
seedsize.csv and seedsize_meta.csv
sites.csv and sites_meta.csv
species.csv


What is measured by individual and species

- for flower parts, the default is at the species level unless the actual piece was collected; Plant parts that were collected on an individual is recorded in the file flowerParts
- all reproductive output is measured at the individual
...
Each species uses notably different algorithms to determine reproductive tissue investment, as each species forms different reproductive tissues and different methods are practical for recording each species' reproductive investment in the field. For some reproductive parts a simple count is pracical, while for species with a large number of buds (or flowers or fruit) that are regularly spaced along branches, the length of stem covered in buds is a better measure. For species that form cones, the cones can be notably different in size and each cone's dimensions must be recorded. 
...
As the reproductive cycle progresses, tissue parts were repeatedly split into two (or more) parts. For instance, a bud, once open, divides into being petals + stamens, a sepal, and a stigma. As a pollinated stigma develops into a fruit, it will be further divided into seed, seed pod, and potentially additional parts. This is shown in the map for each species.
...
\label{tab:species}
\begin{tabular}{p{0.1cm}p{4cm}|p{1.5cm}|p{1.5cm}p{1.5cm}|p{1.5cm}p{1.5cm}|p{1.5cm}p{1.5cm}|p{1.5cm}p{1.5cm}|p{1.5cm}p{1.5cm}}
\hline
& \multicolumn{8}{c|}{Proportion} & \multicolumn{2}{c}{Proportion}& \multicolumn{2}{c}{Proportion} \\
& \multicolumn{8}{c|}{pollen-attraction} & \multicolumn{2}{c}{provisioning}& \multicolumn{2}{c}{success} \\
& \multicolumn{8}{c|}{investment to} & \multicolumn{2}{c}{investment to}& \multicolumn{2}{c}{investment to} \\
<<echo=FALSE, results='asis', warning=FALSE>>=
...
make_table_RA_shifts_with_age <- function() {
  ...
  accessory_vs_RE <- lines(fit$groupsummary)
  accessory_vs_RE <- dplyr::select(accessory_vs_RE,group,n,r2,pval)
  ...
  fit <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="SMA", log="xy")
  accessory_vs_size <- lines(fit$groupsummary)
  accessory_vs_size <- dplyr::select(accessory_vs_size,group,n,r2,pval)
  ...
  fit <- sma(data[[yvar]] ~ data[[xvar]]*data[["species"]], method="SMA", log="xy")
  accessory_vs_age <- lines(fit$groupsummary)
  accessory_vs_age <- dplyr::select(accessory_vs_age,group,n,r2,pval)
...
\begin{tabular}{p{4cm}|x{.94cm}x{1cm}x{1.5cm}|x{.75cm}x{1cm}p{1.5cm}|x{.75cm}x{1cm}p{1.5cm}}
\hline
...
% lines that previously created tableS16 - will delete once new version working
...
make_table_buds_vs_size <- function() {
  ...
  TableS18 <- lines(fit$groupsummary)
  TableS18 <- dplyr::select(TableS18,group,n,r2,pval)
...
\label{tab:propagule_invest_vs_repro}
\begin{tabular}{p{4cm}|x{.94cm}x{1cm}x{1.5cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
...
\label{tab:propagule_invest_vs_repro}
\begin{tabular}{p{4cm}|x{.94cm}x{3cm}x{1.5cm}}
\hline
<<echo=FALSE, results='asis', warning=FALSE>>=
