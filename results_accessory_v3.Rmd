...
plot(asin(sqrt(RA))~asin(sqrt(prop_propagule)),data=subset(SummaryInd,seedset>0&seedset<1), pch=16,col=col.spp[as.factor(species)])
words.bottom.right(mod)
...
plot(asin(sqrt(RA))~(propagule_per_seed),data=subset(SummaryInd,seedset>0&seedset<1), log="x",pch=16,col=col.spp[as.factor(species)])
words.bottom.right.logx(mod)
```
...
par(mfrow=c(2,2), cex=1, omi=c(0.2,0.3,0.3,0.3), mai=c(1.1,1,0.3,0.3)) 
options(scipen=20)
mod <- lm(asin(sqrt(seedset))~log(ReproInv),data=subset(SummaryInd,seedset>0&seedset<1))
plot((ReproInv)~asin(sqrt(seedset)),data=subset(SummaryInd,seedset>0&seedset<1), pch=16,col=col.spp[as.factor(species)],log="y")
words.bottom.right.logy(mod)

mod <- lm(asin(sqrt(prop_propagule))~log(ReproInv),data=subset(SummaryInd,prop_propagule>0))
plot((ReproInv)~asin(sqrt(prop_propagule)),data=subset(SummaryInd,seedset>0&seedset<1), pch=16,col=col.spp[as.factor(species)],log="y")
words.bottom.right.logy(mod)
legend("topright",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")

mod <- lm(log(propagule_per_seed)~log(ReproInv),data=subset(SummaryInd,seedset>0&seedset<1))
plot((ReproInv)~(propagule_per_seed),data=subset(SummaryInd,seedset>0&seedset<1), log="xy",pch=16,col=col.spp[as.factor(species)])
words.bottom.right.logxy(mod)
```
