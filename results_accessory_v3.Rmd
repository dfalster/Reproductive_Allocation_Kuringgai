...
plot(seed_count~asin(sqrt(prop_accessory)),SummaryInd, log="y", pch=16,col=col.spp[as.factor(species)],ylab="",xlab="proportion of reproduction to accessory tissues (asin)",cex=.9)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
...
Results:  
    ...
    - When all 4 variables put into a single model, regardless of "accessory cost" calculation, seedset always by far the most significant correlate   
        * seed size is least frequently a correlate in models, although of course highly significant when analyzed on its own  
...
mod <- lm(log(accessory_per_seed)~(seedset)*log(prepollen_all_inv)*log(total_weight)*log(seed_size),data=subset(SummaryInd,seedset>0&prepollen_all_per_seed>0))
summary(mod)
...
plot(asin(sqrt(prop_accessory))~seedset,SummaryInd, log="x", pch=16,col=col.spp[as.factor(species)],ylab="proportional accessory costs",cex=0.9)
...
###Seedset,seed count by age, size, RA
...
* There is no overall shift in either seedset or proportional allocation to propagules with increasing RA  
```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=7,warning=FALSE}
...
plot(asin(sqrt(RA))~asin(sqrt(seedset)),data=subset(SummaryInd,seedset>0&seedset<1), pch=16,col=col.spp[as.factor(species)])
words.bottom.right(mod)


mod <- lm(asin(sqrt(prop_propagule))~asin(sqrt(RA)),data=subset(SummaryInd,prop_propagule>0))
plot(asin(sqrt(RA))~asin(sqrt(prop_propagule)),data=subset(SummaryInd,seedset>0&seedset<1), log="x", pch=16,col=col.spp[as.factor(species)])
words.bottom.right.logx(mod)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
...
lmfits <- lapply(data, function(x) lm(x[[yvar]] ~  asin(sqrt(x[[xvar]]))))
...
* And there are absolutely no patterns of interest relative to seed count   
    * it doesn't even correlate very well with RA or plant size
...
* Proportion pre-pollination costs completely unrelated to reproductive investment
...
options(scipen=5)
mod <- aov(log(ReproInv)~species*log(prop_propagule),data=subset(SummaryInd,prop_propagule>0))
summary(mod)

options(scipen=20)
mod <- lm(log(ReproInv)~species*log(prop_propagule),data=subset(SummaryInd,prop_propagule>0))
...
```{r, echo=FALSE,message=FALSE,fig.width=28, fig.height=28}
...
* Do plants invest equally more in all reproductive tissues as they get bigger or do they increase allocation to certain types of reproductive tissues more than others?

* If they shifted their proportional investment, there would be a relationship between RA and the proportional investment in 1 (or more) of the 5 reproductive energy pools. If they do not shift their proportional investment, all 5 would have no relationship.  
      * Only "proportional energy to pre-pollination aborted tissues" shows a significant decline with increasing RA - and it has a very convincing decline.   
      * Since no other pool shows an increase in proportional investment, that extra energy must be distributed fairly randomly across other pools

* Simultaneously, all tissue pools should show a positive relationship between actual investment amounts (mg) and RA  
      * However, only "pre-pollination successful tissues" and "energy to propagules" have a convincing r-squared value (across all species)  

* It might be interesting to look at this pattern by species
...
plot(asin(sqrt(prop_propagule))~RA,data=subset(SummaryInd,RA>0&prop_propagule>0), log="x", pch=16,col=col.spp[as.factor(species)],ylab="propagules (as proportion repro inv)")
words.bottom.left.logx(mod)


mod <- lm(log(RA)~log((prepollen_aborted_inv)),data=subset(SummaryInd,RA>0&prepollen_aborted_inv>0))
plot(prepollen_aborted_inv~RA,data=subset(SummaryInd,RA<1&RA>0), pch=16,col=col.spp[as.factor(species)],log="xy",ylab="pre-pollination aborted tissues (mg)")
words.bottom.right.logxy(mod)

mod <- lm(log(RA)~asin(sqrt(prop_prepollen_aborted)),data=subset(SummaryInd,RA>0&prop_prepollen_aborted>0))
plot(asin(sqrt(prop_prepollen_aborted))~RA,data=subset(SummaryInd,RA>0&prop_prepollen_aborted>0), pch=16,col=col.spp[as.factor(species)], log="x",ylab="pre-pollination aborted tissues (as proportion repro inv) (asin)")
words.bottom.right.logx(mod)

mod <- lm(log(RA)~log((prepollen_success_inv)),data=subset(SummaryInd,RA>0&prepollen_success_inv>0))
plot(prepollen_success_inv~RA,data=subset(SummaryInd,RA<1&RA>0), pch=16,col=col.spp[as.factor(species)],log="xy",ylab="pre-pollination successful tissues (mg)")
words.bottom.right.logxy(mod)

mod <- lm(log(RA)~asin(sqrt(prop_prepollen_success)),data=subset(SummaryInd,RA>0&prop_prepollen_success>0))
plot(asin(sqrt(prop_prepollen_success))~RA,data=subset(SummaryInd,RA>0&prop_prepollen_success>0), pch=16,col=col.spp[as.factor(species)], log="x",ylab="pre-pollination successful tissues (as proportion repro inv) (asin)")
words.bottom.right.logx(mod)

mod <- lm(log(RA)~log((packaging_dispersal_inv)),data=subset(SummaryInd,RA>0&packaging_dispersal_inv>0))
plot(packaging_dispersal_inv~RA,data=subset(SummaryInd,RA>0&packaging_dispersal_inv>0), pch=16,col=col.spp[as.factor(species)],log="xy", ylab="energy to packaging and dispersal (mg)")
words.bottom.right.logxy(mod)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")

mod <- lm(log(RA)~asin(sqrt(prop_packaging_dispersal)),data=subset(SummaryInd,RA>0&prop_packaging_dispersal>0))
plot(asin(sqrt(prop_packaging_dispersal))~RA,data=subset(SummaryInd,RA>0&prop_packaging_dispersal>0), log="x", pch=16,col=col.spp[as.factor(species)],ylab="packaging and dispersal (as proportion repro inv)")
words.bottom.left.logx(mod)

mod <- lm(log(RA)~log((postpollen_aborted_inv)),data=subset(SummaryInd,RA>0&postpollen_aborted_inv>0))
plot(postpollen_aborted_inv~RA,data=subset(SummaryInd,RA>0&postpollen_aborted_inv>0), pch=16,col=col.spp[as.factor(species)],log="xy", ylab="energy to tissues aborted postpollination (mg)")
words.bottom.right.logxy(mod)

mod <- lm(log(RA)~asin(sqrt(prop_postpollen_aborted)),data=subset(SummaryInd,RA>0&prop_postpollen_aborted>0))
plot(asin(sqrt(prop_postpollen_aborted))~RA,data=subset(SummaryInd,RA>0&prop_postpollen_aborted>0), log="x", pch=16,col=col.spp[as.factor(species)],ylab="tissues aborted postpollination (as proportion repro inv)")
words.bottom.left.logx(mod)
```
...
* Seedset shows a slight decrease with proportional plant height (the best proxy for "relative size")  
    ...
    * only LEES shows a significant decline

* the same analysis can be done on seedset by age, with similar response    
    * Only HEPU, LEES show significant decline
...
* Only EPMI shows a decline in prop propagule with increasing plant weight  
* No species show an increase in prop propagule with increasing plant weight  
* Of course the proportion of species with prop propagule > 0 increases with plant size
...
mod <- lm((prop_maxH)~log((ReproInv)),data=subset(SummaryInd,prop_maxH>0&ReproInv>0))
plot(ReproInv~prop_maxH,data=subset(SummaryInd,prop_maxH>0&ReproInv>0), pch=16,col=col.spp[as.factor(species)],log="y", ylab="total reproductive investment (mg)")
words.bottom.right.logy(mod)

mod <- lm((prop_maxH)~asin(sqrt(RA)),data=subset(SummaryInd,prop_maxH>0))
plot(asin(sqrt(RA))~prop_maxH,data=subset(SummaryInd,prop_maxH>0),pch=16,col=col.spp[as.factor(species)],ylab="RA")
