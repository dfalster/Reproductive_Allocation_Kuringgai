...
#fixed = BAER, BOLE, EPMI, GRBU, GRSP, HATE, HEPU, LEES, PEPU, PELA, PHPH, PUTU

#nearly fixed = PILI
check <- subset(SummarySpp[["mean"]],species=="BOLE")
check$seed_size
check$embryo_endo_size
check$seed_costs
check$prepollen_costs
check$pack_disp_costs
check$pack_disp_costs_plus
check$pack_disp_early_costs
check$seed_early_costs

parts <- subset(PartsSummary,species=="COER")
parts

invest <- subset(InvestmentByPart,species=="PUTU"&part=="flower_petals")
invest

invest_ind <- subset(InvestmentByPart,individual=="GRSP_802" & (part=="inflorescence_stalk"| part=="inflorescence_stalk_in_fruit"))
invest_ind
weight_total <- sum(invest_ind$weight)
weight_total
count2 <- sum(invest_ind$count)


invest_ind2 <- subset(InvestmentByPart,individual=="COER_807")
invest_ind2
count_total <- sum(invest_ind2$count)
count_total

0.7*weight_total/count_total

ind <- subset(SummaryInd, species=="PUTU"&seed_costs>0)
check3 <- select(ind,individual,prepollen_costs,seed_early_costs,prepollen_costs,pack_disp_costs,pack_disp_gross_costs,pack_disp_early_costs,propagule_per_seed,seed_costs,seed_count,propagule_inv)

View(check3)

  plot(seed_costs ~pack_disp_costs,check3)
points(seed_costs ~ propagule_per_seed,check3,col="red",pch=16)
text(seed_costs ~pack_disp_costs,check3,label=individual)
