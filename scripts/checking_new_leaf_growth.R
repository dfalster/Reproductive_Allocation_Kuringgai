pdf("figs2/new_leaf_growth.pdf", height=8.27, width=11.69)
plot

par(mfrow=c(2,2), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.2,1.1,.4,0.2))

data <- subset(SummaryInd,age>1)
...
plot(leaf_area~diameter,data[[spp]],pch=16,col=col.age(age),log="xy",ylim=c(1,max((data[[spp]]$leaf_area*1.1))),xlim=c(min((data[[spp]]$diameter_0*0.9))+1,max(data[[spp]]$diameter*1.1)))
...
abline(log10(min(data[[spp]]$leaf_area_0)+1),0)
title(main=spp)

plot(leaf_area~age,data[[spp]],pch=16,col=col.age(age),log="y",ylim=c(1,max((data[[spp]]$leaf_area*1.1))),xlim=c(0.3,max(data[[spp]]$age)+1))
...
if (min((data[[spp]]$growth_leaf)) <= 0) {
  plot(growth_leaf~repro_inv,data[[spp]],pch=16,col=col.age(age),log="x")
  abline((-min((data[[spp]]$growth_leaf))),0)
} else {
