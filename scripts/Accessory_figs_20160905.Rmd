```{r}
pdf("ms/Accessory_Costs/scaled_ovule count.pdf", height=6, width=6)
plot
...
mean(subset(SummaryInd$prop_discarded,SummaryInd$prop_discarded>0))

#average proportion of energy going to accessory costs (based on propagules, not seeds) (among individuals that produce seed)
mean(subset(SummaryInd$prop_accessory_using_propagule,SummaryInd$seed_count>0))
mean(subset(SummaryInd$prop_accessory_using_embryo_endo,SummaryInd$seed_count>0))
SummarySpp[["mean"]]$prop_accessory_using_propagule

check <- dplyr::select(SummarySpp[["mean"]],species,prop_accessory_using_embryo_endo,prop_accessory_using_propagule)
check <- dplyr::arrange(check,prop_accessory_using_embryo_endo)
...
#discarded per seed unrelated to seedsize
mod <- sma(discarded_costs ~ embryo_endo_costs, data=subset(SummarySpp[["mean"]]),method="SMA", log="xy",slope.test=1)
summary(mod)
plot(discarded_costs ~ embryo_endo_costs, SummarySpp[["mean"]],log="xy",pch=16,col=col.spp(species))


mod <- sma(prop_pollen_attract_vs_all_repro ~ seedset, data=subset(SummarySpp[["mean"]]),method="SMA", log="x",slope.test=1)
summary(mod)
plot(prop_pollen_attract_vs_all_repro ~ seedset, SummarySpp[["mean"]],log="",pch=16,col=col.spp(species))
...
#more discarded = lower seedset across species - in discussion
mod <- sma(seedset ~ prop_discarded, data=subset(SummarySpp[["mean"]],species!="BAER"),method="SMA", log="",slope.test=1)
summary(mod)
plot(seedset ~ prop_discarded, SummarySpp[["mean"]],log="",pch=16,col=col.spp(species))
...
mod <- sma(prop_pollen_attract_vs_all_repro~seedset,data=subset(SummarySpp[["mean"]]),log="x",slope=1,method="SMA")
summary(mod)
...
mod <- sma(prop_pollen_attract_vs_all_repro~scaled_ovule_count,data=subset(SummarySpp[["mean"]]),log="x",slope=1,method="SMA")
summary(mod)
plot(prop_pollen_attract_vs_all_repro ~ scaled_ovule_count, SummarySpp[["mean"]],log="xy",pch=16,col=col.spp(species))
...
#Table 1. color dots
```{r}
win.metafile("ms/Accessory/Table01_dots.wmf", height=3, width=.2)
plot
...
write.csv(acc_slopes, file="ms/Accessory/Table_SXX_propagule_invest_vs_all_repropro_inv.csv",row.names=FALSE)
```
...
for (i in c("total_weight","propagule_inv","fruit_inv","flower_inv","flower_inv2","success_inv","prepollen_success_inv","pack_disp_success_inv","discarded_inv")) {
...
```{r}
win.metafile("ms/Accessory/Figure01b&c_predictions.wmf", height=3, width=6)
plot
...
win.metafile("ms/Accessory/Figure01def_predictions.wmf", height=6, width=6)
plot
...
#Figure 2
```{r}
win.metafile("ms/Accessory/Figure02.wmf", height=9, width=6)
plot
...
legend("bottomleft",legend=c("seed count by success costs","initiated ovules count by","pollen-attraction costs"),col=c(venetian_red,cerulean,"white"),pch=c(16,16,16),bty="n",cex=0.7)
...
#Figure 2 OLD. Scaling of accessory costs versus weight of embryo + endosperm, by species
```{r}
win.metafile("ms/Accessory_Costs/Figure02_embryo_size_acc_cost_correlations.wmf", height=9, width=10)
plot
...
plot((ovule_count/seed_count)~prop_pollen_attract_vs_all_repro_all_repro,SummarySpp[["mean"]],pch=16,log="xy",col=col.spp(species),ylab="",ylim=c(.8,500),yaxt="n",xlim=c(.0015,.5),xaxt="n")
...
mod_choosy <- sma((ovule_count/seed_count)~prop_pollen_attract_vs_all_repro_all_repro,data=SummarySpp[["mean"]],log="xy",slope=-1,method="SMA")

line_function_log_xy(SummarySpp[["mean"]]$prop_pollen_attract_vs_all_repro_all_repro,mod_choosy,1)
summary(mod_choosy)
...
plot(prop_pollen_attract_vs_all_repro~embryo_endo_costs,SummarySpp[["mean"]],col=col.spp(species),pch=16,log="x", yaxt="n",xaxt="n", ylim=c(0,1), xlim=c(.002,500), ylab="",xlab="",cex=1.5)
points(prop_prepollen_all_vs_all_repro~embryo_endo_costs,SummarySpp[["mean"]],col=col.spp(species),pch=1,cex=1.5)
axis(2, at=c(0,.5,1), labels=c(0,0.5,1.0),cex.axis=1)
axis(1, at=c(0.01,1,100), labels=c(0.01,1,100),cex.axis=1)
extra.top.left.logx("d",2,1.2)

mod_prop_pollen_attract_vs_all_repro <- sma(prop_pollen_attract_vs_all_repro~embryo_endo_costs,SummarySpp[["mean"]],log="x",method="SMA",slope.test=0)
mod_prop_prepollen_all_vs_all_repro <- sma(prop_prepollen_all_vs_all_repro~embryo_endo_costs,SummarySpp[["mean"]],log="x",method="SMA",slope.test=0)

line_function_log_x(SummarySpp[["mean"]]$embryo_endo_costs,mod_prop_pollen_attract_vs_all_repro,1)

summary(mod_prop_pollen_attract_vs_all_repro)
summary(mod_prop_prepollen_all_vs_all_repro)
...
```{r}
win.metafile("ms/Accessory/Figure03_correlations_with_plant_weight.wmf", height=5, width=10)
plot
...
```{r}
win.metafile("ms/Accessory/Figure_SXX_RE_RA_size_age_patterns.wmf", height=10, width=10)
plot
...
write.csv(results, file="ms/Accessory/Table05a_accessory_vs_all_repro.csv",row.names=FALSE)
...
write.csv(results, file="ms/Accessory/Table05b_accessory_vs_total_weight.csv",row.names=FALSE)
...
write.csv(results, file="ms/Accessory/Table05c_accessory_vs_age.csv",row.names=FALSE)
dev.off()
```
...
write.csv(acc_slopes, file="ms/Accessory/Table_S1_extra_flower_invest_vs_all_repro_inv.csv",row.names=FALSE)
```
...
write.csv(results, file="ms/Accessory_Costs/Table07_EXTRA_correlations_with_repro_inv.csv",row.names=FALSE)
```
...
plot(seedset~prop_discarded,data=subset(SummarySpp[["mean"]],species!="BAER"&species!="PEPU"),log="xy",pch=16,col=col.spp(species))
...
```{r}
Cairo("ms/Accessory_Costs/FigureXX_seedcount_reprocount.svg", height=5, width=5,type="svg",pointsize=10,units="in")
#win.metafile("ms/Accessory_Costs/FigureXX_seedcount_reprocount.wmf", height=5, width=5)
...
```{r}
win.metafile("ms/Accessory_Costs/FigureXX_success_vs_discarded.wmf", height=6, width=8)
plot
...
```{r}
win.metafile("ms/Accessory_Costs/FigureXX_EXTRA_acc_cost_by_seed_count.wmf", height=5, width=5)
plot
...
write.csv(results, file="ms/Accessory_Costs/TableXX_seedcount_vs_accessory.csv",row.names=FALSE)
