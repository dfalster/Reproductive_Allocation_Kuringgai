...
#Table 3.correlations between repro inv and propagule inv by species
```{r}
yvar <- "repro_inv"
xvar <- "propagule_inv"
data <- subset(SummaryInd,SummaryInd[[xvar]]>0&SummaryInd[[yvar]]>0)
...
write.csv(results, file="figs2/Accessory_Costs/Table03_propagule_invest_vs_repro_inv.csv",row.names=FALSE)
```

#Table 4. correlations with total repro inv
```{r}
...
write.csv(results, file="figs2/Accessory_Costs/Table04_correlations_with_repro_inv.csv",row.names=FALSE)
```

#Figure 2. Scaling of accessory costs versus weight of embryo + endosperm, by species
```{r}
win.metafile("figs2/Accessory_Costs/Figure04_embryo_size_acc_cost_correlations.wmf", height=9, width=10)
plot

par(mfrow=c(2,2), cex=1, omi=c(1,.1,.05,.05), mai=c(.1,.8,.05,.05)) 
options(scipen = 5)

plot(seedset~embryo_endo_size,SummarySpp[["mean"]],log="xy",col=col.spp(species),pch=16, xlim=c(.02,5000),ylim=c(.004,.5),cex=1.5,xaxt="n",xlab="",ylab="")
...
extra.bottom.left.logxy("a",2,1.2)

plot((scaled_seed_count)~(embryo_endo_size),SummaryInd,pch=16,log="xy",col=col.spp(species),xlim=c(.02,5000),ylim=c(1e-4,1000),yaxt="n",xaxt="n",ylab="")
...
axis(1, at=c(0.1,10,1000), labels=c("","",""),cex.axis=1)
extra.bottom.left.logxy("b",2,1.2)
...
plot(prepollen_costs~embryo_endo_size,SummarySpp[["mean"]],col=col.spp(species),pch=16,log="xy", yaxt="n",xaxt="n", ylim=c(.02,5000), xlim=c(.02,5000), ylab="",xlab="",cex=1.5)
...
plot(postpollen_all_per_seed~embryo_endo_size,SummarySpp[["mean"]],col=col.spp(species),pch=16,log="xy", yaxt="n",xaxt="n", ylim=c(.02,5000), xlim=c(.02,5000), ylab="",xlab="",cex=1.5)
