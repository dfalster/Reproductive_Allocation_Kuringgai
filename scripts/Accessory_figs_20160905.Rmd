plot(scaled_seed_count ~ scaled_bud_count, SummarySpp[["mean"]],log="xy",pch=16,col=col.spp(species),xlim=c(.1,5000),ylim=c(0.001,50))
mod <- sma(scaled_seed_count ~ scaled_bud_count, SummarySpp[["mean"]],log="xy",slope=1,method="OLS")
summary(mod)
line_function_log_xy(SummarySpp[["mean"]]$scaled_bud_count,mod,1)
line_function_log_xy_zero(SummarySpp[["mean"]]$scaled_bud_count,mod,1)

plot(scaled_bud_count ~ seedset, SummarySpp[["mean"]],log="y",pch=16,col=col.spp(species))
...
#proportion energy invested in accessory tissues unrelated to seedsize
mod <- sma(seed_size ~ prop_accessory_ee, data=subset(SummarySpp[["mean"]],species!="BAER"),method="OLS", log="y",slope.test=1)
summary(mod)
plot(seed_size ~ prop_accessory_ee, SummarySpp[["mean"]],log="y",pch=16,col=col.spp(species))
...
points(scaled_bud_count~prop_prepollen_costs,SummarySpp[["mean"]],pch=21,col=col.spp(species))
...
mod2 <- sma(scaled_bud_count~prepollen_costs,data=subset(SummarySpp[["mean"]],scaled_bud_count!=0),log="xy",slope=3.23,method="OLS")
