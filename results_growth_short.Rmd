...
plot((RA+.01)~growth_stem_diameter, SummaryInd, log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)

mod <- lm(log(prop_growth_stem_diameter)~asin(sqrt(RA)),data=subset(SummaryInd,RA>0))
plot((RA+.01)~prop_growth_stem_diameter, SummaryInd, log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
...
SummaryIndPCA <- SummaryInd %>%
  ...
  select(prop_maxH,height,maxH,WD_mean,LMA_mean,seed_size,growth_leaf,
         ...
         RA,seedset,repro_all_count,accessory_inv,propagule_inv,shoot_leaf_area,shoot_growth_leaf_area,RGR)
