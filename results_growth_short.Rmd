...
- More robust curve-fitting techniques are required to better illustrate this pattern (**CURVE FITTING**)
...
mod <- lm(log(growth_stem_area)~asin(sqrt(RA)),data=subset(SummaryInd,RA>0))
plot((RA+.01)~growth_stem_area, SummaryInd, log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
...
mod <- lm(log(growth_height)~asin(sqrt(RA)),data=subset(SummaryInd,RA>0&growth_height>0))
plot((RA+.01)~(growth_height), SummaryInd, log="xy", col=col.spp[as.factor(species)],pch=16)
```
