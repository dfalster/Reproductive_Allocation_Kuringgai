...
```{r, echo=FALSE,message=FALSE}
mod <- lm(LMA_mean~species+log(FinalWeight_mean), SummarySppAge)
summary(mod)
anova(mod)

mod <- lm(LMA_mean~species+log(FinalWeight_mean), data=subset(SummarySppAge,age>2))
summary(mod)
anova(mod)

plot(LMA_mean ~FinalWeight_mean, SummarySppAge, log="xy", col=col.spp[as.factor(species)], pch=16)
legend("topleft",col=col.spp, pch=16,legend=labels.spp,cex=.8,bty="n")

plot(LMA_mean ~FinalWeight_mean, data=subset(SummarySppAge), log="xy", col=col.age[as.factor(age)], pch=16, cex=1)
...
```{r, echo=FALSE,message=FALSE}
plot((LL_death~PropH), data=subset(SummaryInd,LL_death<10), col=col.spp[as.factor(species)],pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")


plot((PropH~log(age)), data=subset(SummaryInd), col=col.spp[as.factor(species)],pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
