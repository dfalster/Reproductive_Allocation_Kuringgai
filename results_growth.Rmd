...
PCA_loadings$PC1 <- PCA_loadings$PC1*PCA_loadings$scale
PCA_loadings$PC2 <- PCA_loadings$PC2*PCA_loadings$scale
PCA_loadings$PC3 <- PCA_loadings$PC3*PCA_loadings$scale

PCA_rotated <- as.data.frame(PCA_mod$x)
PCA_rotated$individual <- subset(SummaryInd$individual, SummaryInd$lvs_end_total>0)
SummaryInd_PCA_results <- merge(SummaryInd,PCA_rotated, by="individual", all.x=TRUE)

plot(PC1~PC2,SummaryInd_PCA_results,pch=16,col=col.spp[as.factor(species)])
text(PC1~PC2,PCA_loadings,labels=variables, pos=2, offset=0, cex=0.9,col="black")
legend("topright",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
PCA_mod$rotation
plot(PCA_mod)
...
looking at reproducing individuals - changes patterns quite a bit, with variables like maxH_spp pulling toward all the "big size" arrows
SummaryIndPCA <- SummaryInd %>%
...
###PCA analyses indicate a few clusters of variables
* one is between RGR vs RA, maxH and various other measures of reproductive investment
* the model shows that indeed RA and maxH explain most of the variation in RGR once species accounted for
* and total plant weight is not significant - whether put before or after "repro_all_count"
* patterns quite different when species removed and other traits that vary by species included
