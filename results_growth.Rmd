...
* Main axis are across species affects (PC1) and within species shifts in RGR vs RA (PC2)
* PC1 is WD vs a whole collection of traits that equate with "big plants" including measures of size, height, and growth increment
...
* PC4 is effectively leaf count on shoots ("lvs_end_total") vs. seed size, shoot leaf area and shoot leaf area growth - to first order, some plants have lots of small leaves and others have fewer leaves but greater leaf area, Not sure how seed size fits in
* amazing that RA and RGR almost completely orthoganal to PC1 and PC4; they are really a set of variables that are little correlated with an inter-specific traits or measures of plant size
...
plot(PC1_scale~(PC2_scale),SummaryInd_PCA_results,pch=16,col=col.spp[as.factor(species)])
...
plot(mean1~species,PCA_by_species,cex.axis=.5,pch=16,ylab="PCA axis 4")
se_function(mean1,se1)
```
...
* PC1 is still WD vs lots of growth variables, but now reproductive investment (propagule_inv and accessory_inv) associate with "big size" not high RA
* PC2 is RA, seedset, and prop_propagule vs. RGR, shoot growth variables and prop_accessory; interesting, but not surprising that prop_propagule and prop_accessory pulled to opposite ends
* PC3 is somewhat similar. It is also prop_accessory and prop_maxH vs seed set and prop_propagule, but it now separates out all teh shoot growth values
* PC4 is now the axis that separates LMA_mean and maxH vs. WD_mean

, with variables like maxH_spp pulling toward all the "big size" arrows
...
mod <- lm(RGR~asin(sqrt(RA))+prop_maxH+log(propagule_inv+1)+log(accessory_inv+1)+log(total_weight)+log(repro_all_count+1)+WD_mean+LMA_mean+maxH_spp,SummaryInd)
anova(mod)
glance(mod)[1]
```
...
* seed_size removed, because no additional explanatory power after WD and maxH_spp included


```{r, echo=FALSE,message=FALSE}
mod <- lm(log(growth_shoot_area)~log(maxH_spp)+WD_mean+log(growth_stem_area)+log(total_weight)+RGR+asin(sqrt(RA))+LMA_mean,SummaryInd)
anova(mod)
glance(mod)
```
...
```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=14}
par(mfrow=c(2,2), cex=1, omi=c(.5,0.3,.1,0.2), mai=c(1.1,1,.6,0.2))
plot(WD_mean~maxH_spp,SummarySpp, log="x",col=col.spp[as.factor(species)],pch=16)
mod <- lm(log(maxH_spp)~WD_mean,SummaryInd)
words.bottom.left.logx(mod)

plot(WD_mean~maxH_spp,data=subset(SummarySpp,species!="PUTU"&species!="PHPH"), log="x",col=col.spp[as.factor(species)],pch=16)
mod <- lm(log(maxH_spp)~WD_mean,data=subset(SummarySpp,species!="PUTU"&species!="PHPH"))
words.bottom.left.logx(mod)
...
plot(seedset_mean~maxH_spp,SummarySpp, log="x",col=col.spp[as.factor(species)],pch=16)
mod <- lm(log(maxH_spp) ~ (seedset_mean),SummarySpp)
words.bottom.left.logx(mod)
