  ...
  for (a in seq_len(n.paths)) {
    ...
    for (i in last.census:2) {
      ...
      for (j in rev(seq_len(n))) {
        ...
        R <- InvestmentInAPartType(TreeList, TreeList_Pred, Element, Progression[1:j], GraphMaps)
...

        {
          ...
          for (l in seq_len(n.xors)) {
            ...
            R <- InvestmentInAPartType(TreeList, TreeList_Pred, XOR.Part[l], XOR.Progression, GraphMaps)
      ...
      # Befor moving to next census (from time t to t-1) check the list of avilable predecessor at time t-1 contain any elements. They will not develop to anything,
      # hence they are lost.
      if ((i > 2) & (i < n.censuses)) {
    ...
    # If there are accesories
      for (k in seq_len(length(Acc.Finals))) {
        ...
        Aux_Progression_to_root <- get.vertex.attribute(Plant.Graph, name = "name", unlist(get.shortest.paths(Plant.Graph, to = Accessory, from = Progression[1],
          weight = NA)$vpath))
        ...
        # Perform investment calculations in the same mannes as in the previous case. Only calculations for elements not on the main progression are made, however all
        ...
        for (i in last.census:2) {
          ...
          R <- InvestmentInAPartType(TreeList, TreeList_Acc, Element.aux, Aux_Progression_to_root[seq_len(length(Aux_Progression_to_root) -
            n.aux + l)], GraphMaps)
  ...
  for (t in (N - 1):2) {
    ...
    for (Predecessor in Progression[ rev(seq_len(length(Progression) - 1))]) {
      ...
      if (sum(Predecessor == parts.at.time.t)) {
        ...
        ErrList <- data.frame(Element = NA, Census = NA, Count = NA)
        return(list(TreeList_Pred = TreeList_Pred, Invest = El_weight - w * Pre_weight, from = Predecessor, to = Element, FromCensus = t - 1, ToCensus = N -
          1, Count = 1, ErrList = ErrList))
        # take away the count from the list and remove the element
      }
    }
  }
  ErrList <- data.frame(Element = Element, Census = N - 1, Count = 1)
  list(TreeList_Pred = TreeList_Pred, Invest = NA, from = NA, to = NA, FromCensus = NA, ToCensus = NA, Count = NA, ErrList = ErrList)
}


# Function that takes the tree structure returned by Weight calculations and transforms it to per/seed counts. The parts are not right now to be understand as
# separate entities rather their fractions that develop to a single seed at the end. Such transformation does not change carbon allocation between the elements
# with the same count, however can allow to adjust for the fact that one fruit can have multiple seeds.
