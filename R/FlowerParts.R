...
make_PartsSummary <- function(FlowerParts, PartsList) {
  list(
    IndividualBasedWeights = make_IndividualBasedWeights(FlowerParts),
    AvCountsPerMM = make_AvCountsPerMM(FlowerParts),
    RegressionTable = make_RegressionTable(FlowerParts),
    AvWeightPerUnit = make_AvWeightPerUnit(FlowerParts, PartsList)
  )
}
...
make_AvCountsPerMM <- function(FPSummary) {
  ...
  if(nrow(data) > 0 ) {
    ret <- group_by(data, species) %>%
      summarise(
        AvCountPerMM=mean(count/length),
        tot.count=sum(count)
      )
    }
 ret
...
# TODO: Would it be better to save these as lm objects? could then use predict function?
...
make_RegressionTable <- function(FPSummary) {
  ...
  regToFit <- data.frame(rbind(
    c("GRBU","fruit_large_immature",  2, "height"),
    c("GRBU","seed_pod",              1, "height"),
    c("GRSP","fruit_large_immature",  2, "height"),
    c("GRSP","seed_pod",              1, "height"),
    c("PELA","fruit_large_immature",  2, "averageWeight"),
    c("PELA","seed_pod",              3, "averageWeight"),
    c("PELA","seed",                  3, "averageWeight"),
    c("BAER","cone_young",            2, "height_noIntercept"),
    c("BAER","cone_green",            1, "volume"),
    c("BAER","cone_brown_no_expanded_follicles", 1, "volume"),
    c("BAER","cone_brown",            1, "volume"),
    c("BAER","cone_aborted",          2, "height_noIntercept"),
    c("BAER","cone_base_green",       1, "volume"),
    c("BAER","cone_base_brown",       1, "volume"),
    c("PEPU","cone_just_starting",    1, "averageWeight"),
