### Removing duplicates from the list - Need to be used if a mixed method was used to obtain the weights at one census - Need to be used for joining the
...
# Functions that transform census data to weights 1. Count to vector weights 2. Length of plant part to count 3. Dimension of individual to it's weight.

# Weights from counts (vector with homogeneous weights)
WeightFromCount <- function(count, species, part, individual, census, AvWeightPerUnit, IBW) {
  if (grepl("0", part)) {
...
WeightFromLength <- function(length, species, part, individual, census, AvCountsPerMM, AvWeightPerUnit, IBW) {
  if (grepl("0", part)) {
    part <- str_sub(string = part, start = 1, end = -4)
  }
  count <- round(length * AvCountsPerMM[AvCountsPerMM$species == species, ]$AvCountPerMM)
  w <- WeightFromCount(count, species, part, individual, census, AvWeightPerUnit, IBW)
  w
}

# Weight from dimensions (for single plant element)
WeightFromRegression <- function(height, diameter, species, part, individual, census, IBW, RegressionTable) {
  if (grepl("0", part)) {
    part <- str_sub(string = part, start = 1, end = -4)
  }
  w <- NA
  ...
  for (i in seq_len(n.parts)) {
    ...
    if (Parts[i, species] == "regress_by_dim") {
      ...
      if (!is.na(height_ch)) {
        ...
        for (j in seq_len(count)) {
          weight[j] <- WeightFromRegression(heights[j], NA, species, type, TreeID, census, IBW, RegressionTable)
        }
        Element <- list(type = type, m.type = m.type, count = length(weight), weight = weight)
        C_list[[k]] <- Element
      }
    ...
    if (Parts[i, species] == "volume") {
      ...
      if (!is.na(dimension_ch)) {
        ...
        for (j in seq_len(count)) {
          weight[j] <- WeightFromRegression(heights[j], diameters[j], species, type, TreeID, census, IBW, RegressionTable)
        }
        Element <- list(type = type, m.type = m.type, count = length(weight), weight = weight)
        C_list[[k]] <- Element
      }
