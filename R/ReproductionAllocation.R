...
RA_Calculations <- function(thisSpecies, Species_Investment, HarvestData, Maps, IndividualsList) {
  ...
  GrowthEst <- HarvestData_basal %>%
    group_by(individual) %>%
    mutate(
           leaf_weight_est =  predict_leaf_weight(diameter, individual[1]),
           ...
           growth_inv =  growth_leaf + growth_stem,
