...
SummarySppAge[["mean"]]$leaf_shed_mean <- SummaryCombine %>%
        group_by(species,age) %>%
        summarise(mean(leaf_shed_mean))

SummarySppAge[["mean"]]$leaf_replacement_mean <- SummaryCombine %>%
        group_by(species,age) %>%
        summarise(mean(leaf_replacement_mean))
...
data <- select(SummarySppAge[["mean"]], species, age, repro_inv, growth_leaf, leaf_replacement,leaf_area_0,leaf_weight_0,total_inv,prop_surplus,
  ...
  LL_death,height_0,total_weight_0,all_leaf_inv,surplus_inv,all_leaf_and_repro_inv,propagule_inv,ovule_count,success_inv)
data$leaf_shed2 <- data$leaf_weight_0 / data$LL_death
data_ind <- select(SummaryCombine,species,individual,age, LL_death,height_0,leaf_shed,leaf_weight_0,propagule_inv,ovule_count,
