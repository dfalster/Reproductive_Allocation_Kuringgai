...
plot(dispersalcosts_increment ~ seed_size,data=subset(SummarySppAge[["mean"]]),col=col.age(age),pch=16,log="xy", ylab="per seed cost of packaging and disp")
...
plot(seed_to_ovule_ratio ~ total_accessory_Lord,data=subset(SummaryInd),col=col.spp(species),pch=16,log="xy")
mod <- lm(log(seed_size) ~ total_accessory_Lord,data=subset(SummarySpp[["mean"]]))
words.bottom.left.logx(mod)



plot(seed_to_ovule_ratio ~ seed_size,data=subset(SummarySpp[["mean"]]),col=col.spp(species),pch=16,log="x")
mod <- lm(log(seed_size) ~ seed_to_ovule_ratio,data=subset(SummarySpp[["mean"]]))
words.bottom.left.logx(mod)
```
