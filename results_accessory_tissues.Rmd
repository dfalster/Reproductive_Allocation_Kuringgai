...
yvar <- "prop_accessory_mean"
xvar <- "seed_size"
data <- SummarySpp

plot_yvar_vs_xvar(data, yvar, xvar,log="x",
                  xlab=xvar, ylab=yvar)
mod <- lm(log(seed_size)~prop_accessory_mean,SummarySpp)
summary(mod)
anova(mod)
```
...
plot_yvar_vs_xvar(data, yvar, xvar,
                 xlab=xvar, ylab=yvar)
mod <- lm(asin(sqrt(prop_propagule)) ~ species-1, data)
summary(mod)
data.frame(
...
tmp2 <- select(SummarySpp, species, prop_propagule_mean, prop_propagule_se)
tmp <- merge(tmp, tmp2, by="species")
tmp
plot(prop_propagule_mean~species,tmp,cex.axis=.8,pch=16,log="y",ylim=c(.001,.25),main="Percent reproductive investment put into propagules")

mean1 <- tmp$prop_propagule_mean
se1 <- tmp$prop_propagule_se

se_function(mean1,se1)

#for(i in 1:14) {
#arrows(i,tmp$prop_propagule_mean[i]+tmp$prop_propagule_se[i],i,tmp$prop_propagule_mean[i]-tmp$prop_propagule_se[i],code=3,angle=90, length=0.1)}


#barplot(height=test$prop_propagule_mean, names=test$species,cex.axis=.8,ylim=c(0,.25),cex.names=.8)
#xv <- barplot(height=test$prop_propagule_mean, names=test$species,cex.axis=.8,ylim=c(0,.25),cex.names=.8)
#xv

#points((prop_propagule_mean+prop_propagule_se)~species,test,pch=25)
#points((prop_propagule_mean-prop_propagule_se)~species,test,pch=24)
...
plot_yvar_vs_xvar(data, yvar, xvar,
                  xlab=xvar, ylab=yvar)
mod <- lm(asin(sqrt(prop_prepollen_aborted)) ~ species, data)
summary(mod)
```

##Do individual species decrease accessory costs as they increase in size, age or RA?
###Change in accesory costs With size:  
- prop_accessory is arcsine transformed, size is log transformed  
...
###Change in accesory costs With age:
- prop_accessory is arcsine transformed, age is log transformed  
...
###Change in accesory costs With respect to RA:  
- both RA and prop_accessory are arcsine transformed  
...
###Change in aborted bud investment with size:  
- prop_prepollen_aborted is arcsine transformed, size is log transformed  
...
###Change in aborted bud investment with respect to age:  
- prop_prepollen_aborted is arcsine transformed, age is log transformed  
...
###Change in aborted bud investment with respect to RA:
- both RA and prop_prepollen_aborted are arcsine transformed  
...
###Do total pre-pollination costs change with respect to size:  
- prop_prepollen_all is arcsine transformed, size is log transformed  
...
data <- filter(SummaryInd,ReproInv>0)
data <- split(data, data$species)
lmfits <- lapply(data, function(x) lm(asin(sqrt(x[[yvar]])) ~  log(x[[xvar]])))
results <- summarise_all_fits(lmfits)
...
###Do total pre-pollination costs change with respect to age:
- prop_prepollen_all is arcsine transformed, age is log transformed  
...
data <- filter(SummaryInd,ReproInv>0&age>2)
data <- split(data, data$species)
lmfits <- lapply(data, function(x) lm(asin(sqrt(x[[yvar]])) ~  log(x[[xvar]])))
results <- summarise_all_fits(lmfits)
...
### Do total pre-pollination costs change with respect to RA:  
- both RA and prop_prepollen_all are arcsine transformed    
...
### Do total pre-pollination costs change with respect to Reproductive Investment:
- prop_prepollen_all is arcsine transformed, ReproInv is log transformed  
...
plot(prop_propagule_mean ~ age, data=data2, xlab="age", ylab="proportion tissue", cex.axis=.8, las=1,
       main=labels.spp[spp], ylim=c(0,1), xlim=c(1,35), log="x", pch=16,col="orange")
points(prop_prepollen_aborted_mean ~ age, data=subset(SummarySppAge, species==spp), cex=1.3, pch=8, col="red")
points(prop_prepollen_success_mean ~ age, data=subset(SummarySppAge, species==spp), cex=1.3, pch=17, col="blue")
points(prop_postpollen_aborted_mean ~ age, data=subset(SummarySppAge, species==spp), cex=1.3, pch=18, col="black")
points(prop_packaging_dispersal_mean ~ age, data=subset(SummarySppAge, species==spp), cex=1.3, pch=19, col="light green")
...
###Considering relative effect of different accessory costs on RA
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(RA))~asin(sqrt(prop_propagule))+asin(sqrt(prop_prepollen_aborted))+ asin(sqrt(prop_prepollen_success))+ asin(sqrt(prop_postpollen_aborted))+ asin(sqrt(prop_packaging_dispersal)) + species,data=subset(SummaryInd,Total>0))

mod <- lm(asin(sqrt(RA))~asin(sqrt(prop_propagule))+asin(sqrt(prop_prepollen_aborted))+ asin(sqrt(prop_prepollen_success))+ asin(sqrt(prop_postpollen_aborted))+ asin(sqrt(prop_packaging_dispersal)) +asin(sqrt(prop_propagule))*species+asin(sqrt(prop_prepollen_aborted))*species+ asin(sqrt(prop_prepollen_success))*species+ asin(sqrt(prop_postpollen_aborted))*species+ asin(sqrt(prop_packaging_dispersal))*species+ species,data=subset(SummaryInd,Total>0))
summary(mod)
plot(mod)
step(mod)
```