...
* Reproductive investment can be considered on an energy or "count" (fecundity) basis
    ...
    2. What might cause shifts in accessory costs across species?
      ...
      * This data set also lets us look at individual pools of accessory tissues and see how they change across species.
...
plot(ReproInv~FinalWeight,data=subset(SummaryInd,ReproInv>0), log="xy",xlab="", col=col.spp[as.factor(species)], pch=16, ylab="Total reproductive investment (mg)",xlim=c(100,2000000))
words.bottom.right.logxy(mod)

mod <- lm (log(FinalWeight)~log(fruit_weight), data=subset(SummaryInd,fruit_weight>0))
plot(fruit_weight~FinalWeight,data=subset(SummaryInd,fruit_weight>0), log="xy",col=col.spp[as.factor(species)], pch=16, ylab="Fruit weight (mg)",xlim=c(100,2000000),xlab="")
words.bottom.right.logxy(mod)

mod <- lm (log(FinalWeight)~log(Prop), data=subset(SummaryInd,Prop>0))
plot(Prop~FinalWeight,data=subset(SummaryInd,Prop>0), log="xy",col=col.spp[as.factor(species)], pch=16, xlab="", ylab="Propagule investment (mg)", xlim=c(100,2000000))
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm (log(FinalWeight)~log(PerPropagule), data=subset(SummaryInd,PerPropagule>0))
plot(PerPropagule~FinalWeight,data=subset(SummaryInd,PerPropagule>0), log="xy",col=col.spp[as.factor(species)], pch=16, ylab="Proportion reproductive investment to propagules",xlim=c(100,2000000),xlab="Total size (mg)")
words.bottom.right.logxy(mod)
text((par("fin")[1]*.5),(par("fin")[2]*.2),"Total size (mg)")
```
...
plot(asin(sqrt(PerAcc_mean))~seed_size,SummarySpp, log="x", pch=16,col=col.spp[as.factor(species)],ylab="Accessory costs (asin transformed)",xlab="seed size (mg)",cex=1.3)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logx(mod)

mod <- lm(log(FinalWeight_mean)~asin(sqrt(PerAcc_mean)),SummarySpp)
plot(asin(sqrt(PerAcc_mean))~FinalWeight_mean,SummarySpp, log="x", pch=16,col=col.spp[as.factor(species)],ylab="",xlab="Total weight (mg)",cex=1.3)
words.bottom.right.logx(mod)

mod <- lm(log(seedset_mean)~asin(sqrt(PerAcc_mean)),SummarySpp)
plot(asin(sqrt(PerAcc_mean))~seedset_mean,SummarySpp, log="x", pch=16,col=col.spp[as.factor(species)],ylab="Accessory costs (asin transformed)",xlab="seed set",cex=1.3)
words.top.right.logx(mod)

mod <- lm(asin(sqrt(seedset_mean))~asin(sqrt(PerPrePol_All_mean)),SummarySpp)
plot(asin(sqrt(PerAcc_mean))~PerPrePol_All_mean,SummarySpp, log="", pch=16,col=col.spp[as.factor(species)],ylab="",xlab="pre-pollination costs (proportional)",cex=1.3)
words.top.right(mod)
```
...
mod <- lm(asin(sqrt(seedset))~log(PerPropagule),data=subset(SummaryInd,seedset>0&seedset<1))
plot(PerPropagule~seedset,data=subset(SummaryInd,seedset>0&seedset<1), log="xy", pch=16,col=col.spp[as.factor(species)],ylab="Accessory costs (asin transformed)",xlab="seed set")
words.bottom.right.logxy(mod)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
...
par(mfrow=c(1,1), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.1,1.1,.1,0.2))  
mod <- lm(asin(sqrt(seedset))~log(PerPropagule),data=subset(SummaryInd,seedset>0&seedset<1))
plot(PerPropagule~seedset,data=subset(SummaryInd,seedset>0&seedset<1), log="xy", pch=16,col=col.age[as.factor(age)],ylab="Accessory costs (asin transformed)",xlab="seed set")
words.bottom.right.logxy(mod)
