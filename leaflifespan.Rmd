...
plot(LMA_mean~age,SummarySppAge,log="xy", xlab="age",ylab="LMA",col=col.spp[as.factor(species)],pch=16)
legend("bottomright",col=col.spp, labels.spp, pch=16, cex=1)
...
legend("bottomright",col=col.spp, labels.spp, pch=16, cex=1)
mod <- lm(lvs_end_total~age+species+age*species,SummaryInd)
summary(mod)
...
plot_RAbyAge <- function(spp) {
  plot(RA~age, data=subset(SummaryInd,species==spp), xlab="age",ylab="reproductive allocation", log="x", main=labels.spp[spp])
  points(RA_mean~age, data=subset(SummarySppAge, species==spp),pch=16,cex=1.2)
}
