...
- make sure regressions exist (or are done), to separately look at leaf and stem biomass
- seed weight vs. propagule weight dilemma
- sites that are aged 1.35 vs 1.4 years
...
names(investment) <- c("individual","ReproInv","age","GrowthInv","StartWeight","FinalWeight","StartBasalDiamAv","FinalBasalDiamAv","TotalInvestment","RA","species")
#still have tagId as label
#remaking this from scratch
...
leaf_lifespan_temp <- leafLifespan %>%
              select(species,age,site,individual,d_start,d_end,length_start,new_length,segment,lvs_start_count,lvs_end_count,lvs_new_count,new_but_shed_count)
...
#SummaryInd <- left_join(leafLifespan, investment, by = "individual")
#SummaryInd <- left_join(SummaryInd, accessory, by = "individual")
#SummaryInd <- left_join(SummaryInd, harvest, by = "individual")
#not sure why the above text isn't working... using merge instead

SummaryInd <- merge(leafLifespan, investment, by.x=c("individual"), by.y=c("individual"))
SummaryInd <- subset(SummaryInd, select = -c(age.y,species.y) )
SummaryInd <- merge(SummaryInd, accessory, by.x=c("individual"), by.y=c("individual"))
SummaryInd <- subset(SummaryInd, select = -c(age,species) )
SummaryInd <- merge(SummaryInd, harvest, by.x=c("individual"), by.y=c("individual"))
#problem here, because duplicates number of entries
SummaryInd <- subset(SummaryInd, select = -c(age.x,species.x) )
...
harvest_summary <- harvest %>%
  group_by(species, age2) %>%
  summarise_each(funs(mean, se, length), total_stem_weight, total_leaf_weight, height)
...
MaxH_spp <- harvest %>%
  group_by(species) %>%
  summarise_each(funs(max), height)
names(LMA_spp)<-c("species", "maxH_spp")


SummaryInd <- merge(SummaryInd,Wood_summary, by.x=c("species"), by.y=c("species"))
SummaryInd <- merge(SummaryInd,LMA_summary, by.x=c("species","age"), by.y=c("species","age"))
SummaryInd <- merge(SummaryInd,MaxH_spp, by.x=c("species"), by.y=c("species"))
SummaryInd <- merge(SummaryInd,seedsize, by.x=c("species"), by.y=c("species"))
SummaryInd$leafArea <- SummaryInd$total_leaf_weight * SummaryInd$LMA_mean
#TO DO: REMOVE DUPLICATE COLUMNS FOR AGE and SPECIES MORE ELEGANTLY
#TO DO: REMOVE COLUMNS I DON'T NEED

#merge various data summaries into SummarySppAge dataframe
SummarySppAge <- merge(LL_summary, LMA_summary, by.x=c("species","age"), by.y=c("species","age"))
SummarySppAge <- merge(SummarySppAge, Wood_summary, by.x=c("species"), by.y=c("species"))
SummarySppAge <- merge(SummarySppAge, seedsize, by.x=c("species"), by.y=c("species"))
SummarySppAge <- merge(SummarySppAge, MaxH_spp, by.x=c("species"), by.y=c("species"))
SummarySppAge <- merge(SummarySppAge, maxH, by.x=c("species","age"), by.y=c("species","age"))
SummarySppAge <- merge(SummarySppAge, investment_summary, by.x=c("species","age"), by.y=c("species","age"))
SummarySppAge <- merge(SummarySppAge, accessory_summary, by.x=c("species","age"), by.y=c("species","age"))
SummarySppAge <- merge(SummarySppAge, harvest_summary, by.x=c("species","age"), by.y=c("species","age"))
#TO DO: REMOVE DUPLICATE COLUMNS FOR AGE

#merge various data summaries into SummarySpp dataframe
SummarySpp <- merge(LL_spp, LMA_spp, by.x=c("species"), by.y=c("species"))
SummarySpp <- merge(SummarySpp, Wood_summary, by.x=c("species"), by.y=c("species"))
SummarySpp <- merge(SummarySpp, seedsize, by.x=c("species"), by.y=c("species"))
SummarySpp <- merge(SummarySpp, maxH, by.x=c("species"), by.y=c("species"))

#WONDERING IF I SHOULD AT THIS POINT HAVE THE DATA FRAME CALLED "sUMMARYINDIVIDUAL" INSTEAD, SINCE NOW CONTAINS FAR MORE THAN LL DATA
...
```{r, echo=FALSE,results='hide', message=FALSE}
mod <- lm(LL_birth ~ age + species +age*species, data=subset(SummaryInd,age>3 & LL_birth!="Inf"))
summary(mod)
...
#models looking at how the relationship between leaf weight (or area) and total plant weight shifts with age
this should be another way to get at the decreasing "leafiness" of plants with age

```{r, echo=FALSE,results='hide', message=FALSE}
plot(leaf_weight~FinalWeight,SummaryInd, log="xy",col=col.age[as.factor(age)], pch=16, cex=0.8)
legend("bottomright",col=col.age, labels.age, pch=16, cex=1)
mod <-lm(FinalWeight~leaf_weight+age+species,SummaryInd)
summary(mod)

plot_LeafWeight <- function(spp) {
  plot(leaf_weight~FinalWeight,data=subset(SummaryInd,species==spp), xlab="total weight (mg)", ylab ="leaf weight (mg)", log="xy",col=col.age[as.factor(age)], pch=16,     cex=0.8,main=labels.spp[spp])
  legend("bottomright",col=col.age, labels.age, pch=16, cex=1)
  mod <-lm(FinalWeight~leaf_weight+age,data=subset(SummaryInd,species==spp))
  abline(mod)
  summary(mod)
}
