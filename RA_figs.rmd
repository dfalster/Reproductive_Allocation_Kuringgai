...
for(spp in names(data)) {
  plot_yvar_vs_xvar(data[[spp]], yvar, xvar,log="",xlab="",ylab="",cex=1.4, xlim=c(0,1), ylim=c(0,1),
...
Summary2 <- SummaryInd
Summary2 <- merge(Summary2,select(results, -n), by=c("species"), all=TRUE)
Summary2$predict_growth <- ((Summary2$total_weight)*(10^(Summary2$b))) + (Summary2$a)
Summary2$cost <- Summary2$predict_growth - Summary2$growth_inv
Summary2$cost_plus_growth <- Summary2$cost + Summary2$growth_inv
plot(total_weight ~ cost_plus_growth,data=subset(Summary2),log="xy",col=col.spp(species),pch=16,xlim=c(1,1000000),ylim=c(1,1000000))
abline(0,1)
...
par(mfrow=c(1,2), cex=1, omi=c(.7,.7,.1,.1), mai=c(1.1,1.1,.4,0.2)) 
plot(asin(sqrt(RA))~growth_stem_diameter,data=subset(SummaryInd,RA>0),log="x",col=col.spp(species), pch=16)
...
plot(asin(sqrt(RA))~growth_shoot_diameter,data=subset(SummaryInd,RA>0),log="x",col=col.spp(species), pch=16)
...
* top variable loadings: scaled_growth_stem_diameter, wood_density,growth_stem_diameter, shoot_leaf_count vs. maxH, LMA_mean, seed_size, RA  
...
* high loadings for growth_stem_area, growth_leaf, LMA_mean, seed_set vs low loadings for shoot_growth_leaf_area, wood_density, prop_maxH, seed_size  
