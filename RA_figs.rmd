...
for(spp in names(data)) {
  plot_yvar_vs_xvar(data[[spp]], yvar, xvar,log="",xlab="",ylab="",cex=1, xlim=c(0,1), 
...
pdf("figs2/cost_of_repro1.pdf", height=12, width=12)
  plot
...
plot(cost_ratio~RA,Summary2,col=col.spp(species),pch=16,log="y",ylab="ratio of predicted growth to actual growth")
mod <- lm(RA~log(cost_ratio),data=subset(Summary2,RA>0))
words.bottom.left.logy(mod)
legend("bottomright",legend=labels.spp(),col=col.spp(),pch=16, cex=.8,bty="n")

plot(cost_ratio~RA,Summary2,col=col.age(age),pch=16,log="y",ylab="ratio of predicted growth to actual growth")
legend("bottomright",legend=labels.age(),col=col.age(),pch=16, cex=.8,bty="n")
dev.off()
```
...
for(spp in names(data)) {
  plot_yvar_vs_xvar(data[[spp]], yvar, xvar,log="y",xlab="",ylab="",cex=1.3,
                    main=spp,col=col.age(data[[spp]]$age))
  }

mtext("ratio of predicted growth to actual growth", 2, outer=TRUE,cex=1.2)
...
for(spp in names(data)) {
  plot_yvar_vs_xvar(data[[spp]], yvar, xvar,log="",xlab="",ylab="",cex=1.3,
                    main=spp,col=col.age(data[[spp]]$age))
  abline(0,1)                  
}
...
SummaryIndPCA <- SummaryInd %>%
  ...
  select(prop_maxH,height,maxH,wood_density,LMA,seed_size,growth_leaf,
         growth_stem_diameter,leaf_area,growth_shoot_diameter,growth_stem_area,shoot_leaf_count,total_weight,
