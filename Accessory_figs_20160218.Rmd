...
win.metafile("figs2/Accessory_Costs/Figure_costs_of_flowers_vs_seed_size.wmf", height=10, width=10)
plot
...
par(mfcol=c(2,2), cex=1, omi=c(.1,.1,.1,.05), mai=c(1,1,.1,.05))
options(scipen = 4)
...
plot(scaled_seed_count~seed_size,SummaryInd,pch=16,log="xy",col=col.spp(species),xlim=c(.01,1000),ylim=c(1e-6,10),yaxt="n")
...
axis(2,yaxp=c(0.00001,0.0001,0.01,1))
...
plot(scaled_failure_count~prop_prepollencosts,SummarySpp[["mean"]],pch=2,log="y",col=col.spp(species),xlim=c(-.2,1),ylim=c(1e-6,10),cex=1.5)
mod <- sma(scaled_failure_count~prop_prepollencosts,data=subset(SummarySpp[["mean"]]),log="y",slope=1,method="OLS")
sma_abline(mod)
sma_sum(mod)
par("yaxp")

#species that produce a disproportionte number of seeds can do so because they have proportionately lower dispersal costs
plot(scaled_seed_count~prop_dispersalcosts,SummarySpp[["mean"]],pch=16,log="y",col=col.spp(species),xlim=c(-.2,1),ylim=c(1e-6,10),cex=1.5)
mod <- sma(scaled_seed_count~prop_dispersalcosts,data=subset(SummarySpp[["mean"]]),log="y",slope=1,method="OLS")
sma_abline(mod)
sma_sum(mod)
...
```{r}
plot(invest_per_leaf_area~seed_size,SummaryInd, col=col.spp(species),pch=16,log="xy")
points(invest_per_leaf_area~seed_size,SummarySpp[["mean"]], cex=2, col=col.spp(species),pch=16,log="xy")
mod <-sma(invest_per_leaf_area~seed_size,SummarySpp[["mean"]],log="xy",method="OLS")
summary(mod)
```
