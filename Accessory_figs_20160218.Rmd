plot(failure_inv~seed_size,SummaryInd,pch=16,log="xy",col=col.spp(species))
plot(abortedcosts~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),pch=16,log="xy",col=col.spp(species))
mod <- sma(abortedcosts~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),log="xy",method="OLS",slope=1)
summary(mod)

plot(prepollen_failure_per_seed~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),pch=16,log="xy",col=col.spp(species))
mod <- sma(prepollen_failure_per_seed~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),log="xy",method="OLS",slope=1)
summary(mod)

plot(scaled_failure_count~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),pch=16,log="xy",col=col.spp(species))
mod <- sma(bud_failed_count~seed_size,data=subset(SummaryInd,abortedcosts>1e-8),log="xy",method="OLS",slope=1)
summary(mod)

plot(seedset~prop_prepollencosts,data=subset(SummaryInd,seedset>0),pch=16,log="y",col=col.spp(species))
legend("bottomright",legend=labels.spp(),col=col.spp(),pch=16, cex=.9,bty="n")
mod <-sma(seedset~prop_prepollencosts,data=subset(SummarySpp[["mean"]],seedset>0&species!="PHPH"&species!="PUTU"),log="",method="OLS")
summary(mod)
sma_abline(mod)

write.csv(PartsSummary, file="figs2/Accessory_Costs/PartsSummary.csv",row.names=FALSE)

means, counts to include in manuscript
```{r}
count(SummaryInd)
...
#Table 1. correlations with total repro inv
```{r}
...
fit <- sma(data[[yvar]] ~ data[[xvar]], method="OLS", log="xy",slope=1)
...
write.csv(results, file="figs2/Accessory_Costs/Table01_correlations_with_repro_inv.csv",row.names=FALSE)
```

#Figure 2. Correlations of various reproductive investment categories against plant weight
```{r}
...
#Table 3 showing accessory cost values for the species
```{r}
...
write.csv(acc_costs_spp, file="figs2/Accessory_Costs/Table03_AccCosts_by_Species.csv",row.names=FALSE)
```

#Figure 3. Ternary diagram showing seedcosts
```{r}
...
#Table 4 showing seedcost breakdown into components, by species
```{r}
...
#Table 5. Slopes for various seed size by acc cost regressions
```{r}
...
slopes <- sma(log10(postpollen_aborted_per_seed) ~ log10(seed_size),data=subset(SummarySpp[["mean"]],species!="COER"&species!="BAER"),slope.test=1)
abline(slopes$groupsummary["Int"][[1]],slopes$groupsummary["Slope"][[1]])
abline(slopes$groupsummary["Int"][[1]],1,col="red")
acc_by_species <- dplyr::bind_rows(acc_by_species, select(slopes$groupsummary,r2,pval,Slope,Slope_lowCI,Slope_highCI,Int,Int_lowCI,Int_highCI,Slope_test_p))
...
words.top.left.logxy(mod)
slopes <- sma(log10(packaging_dispersal_per_seed) ~ log10(seed_size),data=subset(SummarySpp[["mean"]],species!="LEES"),slope.test=1)
abline(slopes$groupsummary["Int"][[1]],slopes$groupsummary["Slope"][[1]])
abline(slopes$groupsummary["Int"][[1]],1,col="red")
acc_by_species <- dplyr::bind_rows(acc_by_species, select(slopes$groupsummary,r2,pval,Slope,Slope_lowCI,Slope_highCI,Int,Int_lowCI,Int_highCI,Slope_test_p))
mtext(text="seed size (mg)",side=1,line=1.5,outer=TRUE)
...
write.csv(acc_by_species, file="figs2/Accessory_Costs/Table05_acc_cost_seed_size_slopes.csv",row.names=FALSE)
```
...
#Table 6. Higher RE, RA, plant size do not lead to lower accessory costs; age shows mixed results
```{r}
...
write.csv(results, file="figs2/Accessory_Costs/Table06a_accessory_vs_RE.csv",row.names=FALSE)
...
write.csv(results, file="figs2/Accessory_Costs/Table06b_accessory_vs_total_weight.csv",row.names=FALSE)
...
write.csv(results, file="figs2/Accessory_Costs/Table06c_accessory_vs_age.csv",row.names=FALSE)
dev.off()
```
...
#correlations between repro inv and propagule inv by species
```{r}
yvar <- "repro_inv"
xvar <- "propagule_inv"
data <- subset(SummaryInd,SummaryInd[[xvar]]>0&SummaryInd[[yvar]]>0)
...
write.csv(results, file="figs2/Accessory_Costs/Table02_extra_propagule_invest_vs_repro_inv.csv",row.names=FALSE)
```
