...
win.metafile("figs2/Accessory_Costs/Figure_cost_of_flowers_vs_seed_size.wmf", height=4, width=12)
plot



# species with bigger seeds abort a disprortionate # of buds
# plot(seed_size~scaled_failure_count,SummarySpp[["mean"]],pch=16,log="xy",col=col.spp(species),ylim=c(.01,1000),xlim=c(.00001,100))
# mod <- sma(seed_size~scaled_failure_count,data=subset(SummarySpp[["mean"]]),log="xy",slope=-1,method="OLS")
# sma_abline(mod)
# sma_sum(mod)
# 
# species with bigger seeds produce disproportionately few seeds
# plot(seed_size~scaled_seed_count,SummarySpp[["mean"]],pch=16,log="xy",col=col.spp(species),ylim=c(.01,1000),xlim=c(.00001,100))
# mod <- sma(seed_size~scaled_seed_count,data=subset(SummarySpp[["mean"]]),log="xy",slope=1,method="OLS")
# sma_abline(mod)
# sma_sum(mod)

par(mfcol=c(1,3), cex=1, omi=c(.1,.1,.1,.05), mai=c(1,1,.1,.05))
options(scipen = 4)
...
plot(scaled_seed_count~seed_size,SummaryInd,pch=16,log="xy",col=col.spp(species),xlim=c(.01,1000),ylim=c(1e-6,10),yaxt="n",xaxt="n")
...
plot(seedset~prop_prepollencosts,SummarySpp[["mean"]],pch=16,log="y",col=col.spp(species),xlim=c(-.2,1),cex=1.5)
mod <- sma(seedset~prop_prepollencosts,data=subset(SummarySpp[["mean"]]),log="y",slope=1,method="OLS")
sma_abline(mod)
sma_sum(mod)


#species that produce a disproportionte number of seeds can do so because they have proportionately lower dispersal costs
plot(seedset~prop_dispersalcosts,SummarySpp[["mean"]],pch=16,log="y",col=col.spp(species),xlim=c(-.2,1),cex=1.5)
mod <- sma(seedset~prop_dispersalcosts,data=subset(SummarySpp[["mean"]]),log="y",slope=1,method="OLS")
sma_abline(mod)
sma_sum(mod)
...
```{r}
acc_costs_spp <- dplyr::select(SummarySpp[["mean"]],species,seed_size,seedset,accessory_per_seed,prepollencosts,prepollen_all_per_seed,postpollen_aborted_per_seed,packaging_dispersal_per_seed,prop_accessory_nonzero)
...
slopes <- sma(accessory_per_seed ~ seed_size,data=subset(SummarySpp[["mean"]]),slope.test=1,log="xy")
abline(slopes$groupsummary["Int"][[1]],slopes$groupsummary["Slope"][[1]])
#abline(slopes$groupsummary["Int"][[1]],1,col="red")
abline(0,1,col="red")
words.bottom.left.logxy(mod)
...
words.top.left.logxy(mod)
slopes <- sma(log10(packaging_dispersal_per_seed) ~ log10(seed_size),data=subset(SummarySpp[["mean"]]),slope.test=1)
abline(slopes$groupsummary["Int"][[1]],slopes$groupsummary["Slope"][[1]])
abline(slopes$groupsummary["Int"][[1]],1,col="red")
acc_by_species <- dplyr::bind_rows(acc_by_species, select(slopes$groupsummary,r2,pval,Slope,Slope_lowCI,Slope_highCI,Int,Int_lowCI,Int_highCI,Slope_test_p))
...
#species with bigger seeds, lower seedset spend less per flower. Here taking the prop of the cost to make a single flower that goes into pre-pollination parts
```{r}
...
```{r}
xvar <- "seed_size"
yvar <- "prop_prepollencosts"
data <- subset(SummarySpp[["mean"]])
plot(prop_prepollencosts~seed_size,data=subset(SummarySpp[["mean"]]),col=col.spp(species),pch=16,log="xy",cex=1.5,xlab="",ylab="prop seedcosts for prepollination")
...
yvar <- "prop_dispersalcosts"
plot(prop_dispersalcosts~seed_size,data=subset(SummarySpp[["mean"]]),col=col.spp(species),pch=16,log="xy",cex=1.5,xlab="",ylab="prop seedcosts for dispersal")
...
mod_LEES <- lm(log10(accessory_per_seed) ~ (log10(prepollen_all_per_seed)+(log10(postpollen_aborted_per_seed))),data=subset(SummaryInd,SummaryInd[[xvar2]]>0&SummaryInd[[xvar3]]>0&species=="LEES"))
mod_BAER <- lm(log10(accessory_per_seed) ~ (log10(prepollen_all_per_seed)+(log10(packaging_dispersal_per_seed))),data=subset(SummaryInd,SummaryInd[[xvar2]]>0&SummaryInd[[xvar1]]>0&species=="BAER"))
mod_COER <- lm(log10(accessory_per_seed) ~ (log10(prepollen_all_per_seed)+(log10(packaging_dispersal_per_seed))),data=subset(SummaryInd,SummaryInd[[xvar2]]>0&SummaryInd[[xvar1]]>0&species=="COER"))

species_coef <- as.data.frame(c("intercept","packaging&dispersal","prepollen_all_per_seed","postpollen_aborted_per_seed"))
...
BAER_c$effect <- c("intercept","prepollen_all_per_seed","packaging&dispersal")
...
COER_c$effect <- c("intercept","prepollen_all_per_seed","packaging&dispersal")
...
LEES_c$effect <- c("intercept","prepollen_all_per_seed","postpollen_aborted_per_seed")
...
species_t <- as.data.frame(c("intercept","packaging&dispersal","prepollen_all_per_seed","postpollen_aborted_per_seed"))
