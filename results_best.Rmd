...
```{r, echo=FALSE,message=FALSE, fig.width=12, fig.height=6, out.width=792, out.height=576}
yvar <- "RA"
xvar <- "PropH"
...
mod <- lm(log(FinalWeight) ~ (log(PerPropagule))+age,data=subset(SummaryInd,PerPropagule>0))
summary(mod)
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(FinalWeight) ~ (log(PropAllocation))*age,data=subset(SummaryInd,PerPropagule>0))
...
mod <- lm(log(FinalWeight)~log(seedset),data=subset(SummaryInd,seedset>0))
plot(seedset ~ FinalWeight,data=subset(SummaryInd,seedset>0),
...
words.bottom.right.logxy(mod)
mod <- lm(log(FinalWeight)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count ~ FinalWeight,data=subset(SummaryInd,seed_count>0),
          col=col.spp[as.factor(species)],
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(FinalWeight)~log(PerPropagule),data=subset(SummaryInd,seedset>0))
plot(PerPropagule~FinalWeight,data=subset(SummaryInd,PerPropagule>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(PerPrePol_All)) ~log(FinalWeight)*age, data=subset(SummaryInd,PerPrePol_All>0))
anova(mod)
```

- There is however a decrease in RA with plant size at a given age (across species effect)
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(RA)) ~ log(FinalWeight)*age, data=subset(SummaryInd,RA>0))
plot(asin(sqrt(RA))~FinalWeight,data=subset(SummaryInd,seedset>0),col=col.spp[as.factor(species)],pch=16,log="x")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logx(mod)
anova(mod)
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(FinalWeight)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count~FinalWeight,data=subset(SummaryInd,seed_count>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```

```{r, echo=FALSE, message=FALSE}
mod <- lm(log(FinalWeight)~log(fruit_weight),data=subset(SummaryInd,fruit_weight>0))
plot(fruit_weight~FinalWeight,data=subset(SummaryInd,fruit_weight>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```
