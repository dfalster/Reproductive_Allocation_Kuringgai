...
```{r, echo=FALSE, message=FALSE}
plot(PerPropagule ~FinalWeight, data=subset(SummaryInd,RA>00),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")

mod <- lm(log(FinalWeight) ~ asin(sqrt(PerPropagule))*species,data=subset(SummaryInd,ReproInv>0))
summary(mod)
anova(mod)

plot(PerPropagule_mean ~FinalWeight_mean, data=subset(SummarySppAge,RA_mean>0),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")

mod <- lm(log(FinalWeight_mean) ~ asin(sqrt(PerPropagule_mean))*species,data=subset(SummarySppAge,RA_mean>0))
#summary(mod)
anova(mod)

mod <- lm(log(FinalWeight) ~asin(sqrt(PerPropagule))*asin(sqrt(RA))*species,data=subset(SummaryInd,ReproInv>0))
anova(mod)
```
