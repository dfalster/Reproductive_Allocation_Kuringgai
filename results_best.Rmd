...
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),10^((0.93*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("r squared = ",output[1]),adj=0,cex=.9)
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),10^((0.88*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("p-value = ",output[2]),adj=0,cex=.9)
...
output <- data.frame(select(glance(x), r.squared, p.value))
output <- round(output,4)
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.93*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("r squared = ",output[1]),adj=0,cex=.9)
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.88*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("p-value = ",output[2]),adj=0,cex=.9)
...
output <- data.frame(select(glance(x), r.squared, p.value))
output <- round(output,4)
text(((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.93*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("r squared = ",output[1]),adj=0,cex=.9)
text(((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.88*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("p-value = ",output[2]),adj=0,cex=.9)
...
output <- data.frame(select(glance(x), r.squared, p.value))
output <- round(output,4)
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),10^((0.12*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("r squared = ",output[1]),adj=0,cex=.9)
text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),10^((0.07*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("p-value = ",output[2]),adj=0,cex=.9)
...
words.bottom.right.logx <- function (x) {
  output <- data.frame(select(glance(x), r.squared, p.value))
  output <- round(output,4)
  text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.12*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("r squared = ",output[1]),adj=0,cex=.9)
  text(10^((0.75*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),((0.07*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),paste("p-value = ",output[2]),adj=0,cex=.9)
...
plot((ReproInv)~(PropH), SummaryInd, log="y",col=col.spp[as.factor(species)],pch=16,xlim=c(4e-03,1),xlab=xvar)
...
* Reproductive investment vs. PropH
```{r, echo=FALSE,message=FALSE, fig.width=7, fig.height=7, out.width=792, out.height=576}
yvar <- "ReproInv"
xvar <- "PropH"
...
plot(ReproInv~total_weight,data=subset(SummaryInd,ReproInv>0), log="xy",col=col.spp[as.factor(species)], pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm (log(total_weight)~log(Prop), data=subset(SummaryInd,Prop>0))
plot(Prop~total_weight,data=subset(SummaryInd,Prop>0), log="xy",col=col.spp[as.factor(species)], pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm (log(total_weight)~log(Prop+PD), data=subset(SummaryInd,Prop>0))
plot((Prop+PD)~total_weight,data=subset(SummaryInd,Prop>0), log="xy",col=col.spp[as.factor(species)], pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm (log(total_weight)~log(PrePol_All), data=subset(SummaryInd,PrePol_All>0))
plot((PrePol_All)~total_weight,data=subset(SummaryInd,PrePol_All>0), log="xy",col=col.spp[as.factor(species)], pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```
...
- Species that are bigger (by weight) invest a lower proportion of their reproduction in propagules  
   ...
   - There is also an increase in propagule investment with age
```{r, echo=FALSE,message=FALSE, fig.width=14, fig.height=7, out.width=792, out.height=576}
mod <- lm(log(total_weight) ~ (log(PerPropagule))+age,data=subset(SummaryInd,PerPropagule>0))
summary(mod)
plot(PerPropagule ~total_weight, data=subset(SummaryInd,RA>0&PerPropagule>0),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
text(10^((0.70*(par("usr")[2]-par("usr")[1]))+par("usr")[1]),10^((0.85*(par("usr")[4]-par("usr")[3]))+par("usr")[3]),"includes age effect",adj=0) 
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight) ~ (log(PropAllocation))*age,data=subset(SummaryInd,PerPropagule>0))
plot(PropAllocation ~total_weight, data=subset(SummaryInd,RA>0&PerPropagule>0),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
text(1000,.6e-4,"includes age effect (increase in Prop Allocation with age)",adj=0)
summary(mod)
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight) ~ (log(RA)),data=subset(SummaryInd,RA>0))
plot(RA ~total_weight, data=subset(SummaryInd,RA>0),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
anova(mod)

mod <- lm(log(total_weight) ~ (log(RA)),data=subset(SummaryInd,RA>5e-3&age==7))
plot(RA ~total_weight, data=subset(SummaryInd,RA>5e-3&age==7),log="xy", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
```

- A model including the effect of both PerPropagule and RA on total_weight confirms this - there is a strong species effect and strong PerPropagule effect, but RA is much less significant  
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight) ~ (log(RA))*log(PerPropagule)*species,data=subset(SummaryInd,PerPropagule>0))
anova(mod)
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight) ~ asin(sqrt(PerPrePol_All)),data=subset(SummaryInd,PerPrePol_All>0&age>3))
plot(asin(sqrt(PerPrePol_All)) ~ total_weight, data=subset(SummaryInd,PerPrePol_All>0),log="x", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight) ~ asin(sqrt(PerPackDisp)),data=subset(SummaryInd,PerPackDisp>0))
plot(asin(sqrt(PerPackDisp)) ~total_weight, data=subset(SummaryInd,PerPackDisp>0),log="x", col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logx(mod)
```
...
mod <- lm(log(total_weight)~asin(sqrt(PerPropagule)),data=subset(SummaryInd,PerPropagule>0))
plot(asin(sqrt(PerPropagule))~log(total_weight), data=subset(SummaryInd,PerPropagule>0),col=col.spp[as.factor(species)],pch=16)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right(mod)
```
...
##Looking at 
- Species that are smaller have lower seed set -> this is likely what is causing Propagule allocation to decrease with size
- Also a small increase in seedset with age - but a smaller effect 

```{r, echo=FALSE, message=FALSE}
par(mfrow=c(2,2), cex=1, omi=c(.1,0.1,0.1,0.1), mai=c(1.2,1,0.1,0.1))
...
plot(seedset ~ total_weight,data=subset(SummaryInd,seedset>0),
          ...
          pch=16,log="xy",xlab="",ylab="seed set")
words.bottom.right.logxy(mod)

mod <- lm(log(RA)~log(seedset),data=subset(SummaryInd,seedset>0))
plot(seedset ~ RA,data=subset(SummaryInd,seedset>0),
          col=col.spp[as.factor(species)],
          pch=16,log="xy",ylab="",xlab="")
words.bottom.right.logxy(mod)
mod <- lm(log(total_weight)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count ~ total_weight,data=subset(SummaryInd,seed_count>0),
          col=col.spp[as.factor(species)],
          pch=16,log="xy",xlab="plant weight",ylab="seed count")
words.bottom.right.logxy(mod)
mod <- lm(log(RA)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count ~ RA,data=subset(SummaryInd,seed_count>0),
          col=col.spp[as.factor(species)],
          pch=16,log="xy",ylab="")
words.bottom.right.logxy(mod)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight)~log(PerPropagule),data=subset(SummaryInd,seedset>0))
plot(PerPropagule~total_weight,data=subset(SummaryInd,PerPropagule>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
```

- However, as shown above, there is no clear shift across plant size in pre-pollination allocation   
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(PerPrePol_All)) ~log(total_weight)*age, data=subset(SummaryInd,PerPrePol_All>0))
anova(mod)
```

- There is however a decrease in RA with plant size at a given age (across species effect)
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(RA)) ~ log(total_weight)*age, data=subset(SummaryInd,RA>0))
plot(asin(sqrt(RA))~total_weight,data=subset(SummaryInd,seedset>0),col=col.spp[as.factor(species)],pch=16,log="x")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logx(mod)
anova(mod)
```

- Together this indicates that increased propagule allocation is due to increased seedset, which increases RA in smaller-sized species
```{r, echo=FALSE, message=FALSE}
mod <- lm(asin(sqrt(RA))~age*species,data=subset(SummaryInd,RA>0))
anova(mod)
```

- seedset and Proportional allocation to propagules (vs. all accessory tissues) are well correlated, within and across species  
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(seedset)~log(PerPropagule),data=subset(SummaryInd,seedset>0))
plot(PerPropagule~seedset,data=subset(SummaryInd,seedset>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
...
- however, seedset and seed size are much less well correlated
- so for this collection of species, shifts in accessory costs are closely linked to seed_set
- seed set therefore clearly plays a role in the (small) shifts in RA with plant size
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(seedset)~log(seed_size),data=subset(SummaryInd,seedset>0))

plot(seed_size~seedset,data=subset(SummaryInd,PerPropagule>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.top.right.logxy(mod)
```

- there is no relationship between seed_count and Final Weight
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(total_weight)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count~total_weight,data=subset(SummaryInd,seed_count>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```
...
plot(fruit_weight~total_weight,data=subset(SummaryInd,fruit_weight>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```
...
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(ReproInv)~log(fruit_weight),data=subset(SummaryInd,fruit_weight>0))
plot(fruit_weight~ReproInv,data=subset(SummaryInd,fruit_weight>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
```

- producing more seeds has a limited effect on RA  
- it is proprtion of flowers/buds that become seeds (seed set) that matters
```{r, echo=FALSE, message=FALSE}
mod <- lm(log(RA)~log(seed_count),data=subset(SummaryInd,seed_count>0))
plot(seed_count~RA,data=subset(SummaryInd,seed_count>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
...
- However, producing more reproductive units increases RA  - this is a within species effect mostly  
```{r, echo=FALSE, message=FALSE}
