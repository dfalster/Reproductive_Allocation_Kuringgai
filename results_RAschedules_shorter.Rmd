...
words.top.right.logxy(mod)
plot(RA_mean~maxH, SummarySpp, col=col.spp[as.factor(species)],pch=16,log="xy",ylab="")
mod <- lm(log(maxH)~asin(sqrt(RA_mean)), data=subset(SummarySpp,RA_mean>0))
words.bottom.left.logxy(mod)
...
```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=14}
par(mfrow=c(2,2), cex=1, omi=c(.6,0.3,.1,0.2), mai=c(1.1,1,.5,0.2))
plot(maxH~WD_mean,data=subset(SummarySpp), col=col.spp[as.factor(species)],pch=16,log="y",ylab="")
...
words.top.right.logxy(mod)
plot(mature_RA_mean~maxH, data=subset(SummarySpp), col=col.spp[as.factor(species)],pch=16,log="xy",ylab="")
mod <- lm(log(maxH)~asin(sqrt(mature_RA_mean)), data=subset(SummarySpp,species!="BAER"))
words.bottom.left.logxy(mod)
mtext("RA (mature plants only)",2,outer=TRUE)
```

```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=14}
mod <- lm(log(growth_stem_diam) ~ age*WD_mean*LMA_mean*maxH*RA, SummaryInd)
anova(mod)
...
words.top.right.logxy(mod)
plot(RA~maxH, SummaryInd, col=col.spp[as.factor(species)],pch=16,log="xy",ylim=c(.0008,1))
points((RA+.001)~maxH,data=subset(SummaryInd,RA==0),col=col.spp[as.factor(species)],pch=16, cex=spp_zero_RA$RAcex)
