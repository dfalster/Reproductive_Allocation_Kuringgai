...
mod <- lm((PropH)~log(ReproInv), data=subset(SummaryInd,ReproInv>0))
...
* Therefore, there is a negative correlation between PropH (proportion of a species' maximum height) and RA; highly significant, but very low R^2
...
mod <- lm((PropH)~asin(sqrt(RA)), data=subset(SummaryInd,ReproInv>0))
plot((asin(sqrt(RA)))~(PropH), SummaryInd,col=col.spp[as.factor(species)],pch=16,xlim=c(4e-03,1))
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
...
- RA schedules plotted against height are particularly good for a number of species such as EPMI, COER, PELA  
  1. they also show that for PILI that among individuals that reproduce, the "potential" RA decreases with increasing propH  
