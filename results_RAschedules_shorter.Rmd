...
plot(repro_inv~total_weight, SummaryInd, log="xy",col=col.spp[as.factor(species)],pch=16)
...
plot(repro_inv~height, SummaryInd, log="xy",col=col.spp[as.factor(species)],pch=16)
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm((prop_maxH)~log(repro_inv), data=subset(SummaryInd,repro_inv>0))
plot((repro_inv)~(prop_maxH), SummaryInd, log="y",col=col.spp[as.factor(species)],pch=16,xlim=c(4e-03,1))
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
...
plot((1+repro_inv)~growth_inv, SummaryInd, log="xy", col=col.spp[as.factor(species)],pch=16)
...
```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=14}
mod <- lm(log(growth_stem_diameter) ~ age*WD_mean*LMA_mean*maxH*RA, SummaryInd)
anova(mod)
