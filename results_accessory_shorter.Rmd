...
plot(Prop~total_weight,data=subset(SummaryInd,Prop>0), log="xy",col=col.spp[as.factor(species)], pch=16, xlab="", ylab="Propagule investment (mg)", xlim=c(100,2000000))
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)

mod <- lm (log(total_weight)~log(PerPropagule), data=subset(SummaryInd,PerPropagule>0))
plot(PerPropagule~total_weight,data=subset(SummaryInd,PerPropagule>0), log="xy",col=col.spp[as.factor(species)], pch=16, ylab="Proportion reproductive investment to propagules",xlim=c(100,2000000),xlab="Total size (mg)")
words.bottom.right.logxy(mod)
...
mod <- lm(log(total_weight_mean)~asin(sqrt(PerAcc_mean)),SummarySpp)
plot(asin(sqrt(PerAcc_mean))~total_weight_mean,SummarySpp, log="x", pch=16,col=col.spp[as.factor(species)],ylab="",xlab="Total weight (mg)",cex=1.3)
words.bottom.right.logx(mod)
