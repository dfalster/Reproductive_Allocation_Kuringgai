...
plot(seedset~total_weight,data=subset(SummaryInd,prop_propagule>0), log="xy",col=col.spp[as.factor(species)], pch=16, ylab="seedset",xlim=c(100,2000000),xlab="Total size (mg)")
words.bottom.right.logxy(mod)
...
words.top.right(mod)
mtext("Proportion accessory investment (asin transformed)",2, outer=TRUE)
```
###Looking at investment vs. costs per successful seed
```{r, echo=FALSE,message=FALSE,fig.width=14, fig.height=14}
par(mfrow=c(2,2), cex=1, omi=c(.1,0.3,.5,0.2), mai=c(1.1,1,.1,0.2))  
...
plot(accessory_per_seed_mean~total_weight_mean,SummarySpp, log="xy", pch=16,col=col.spp[as.factor(species)],ylab="",xlab="Total weight (mg)",cex=1.3)
words.bottom.right.logxy(mod)
...
words.top.right(mod)
mtext("Accessory investment per successful seed (log scale)",2, outer=TRUE)
```
...
plot(seed_size~seedcosts,data=subset(SummarySpp),log="xy",pch=16,col=col.spp[as.factor(species)])
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")

par(mfrow=c(1,2), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.1,1.1,.1,0.2))
plot(seed_size~costs_per_seed,data=subset(SummarySpp),log="y",pch=16,col=col.spp[as.factor(species)])
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
```
...
par(mfrow=c(1,2), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.1,1.1,.1,0.2))  
mod <- lm(log(seedset)~(prop_accessory),data=subset(SummaryInd,seedset>0&seedset<1))
plot(prop_accessory~seedset,data=subset(SummaryInd,seedset>0&seedset<1), log="x", pch=16,col=col.spp[as.factor(species)],xlab="seed set")
words.bottom.right.logx(mod)

mod <- lm(log(seedset)~log(accessory_per_seed),data=subset(SummaryInd,seedset>0&seedset<1&accessory_per_seed>0))
plot(accessory_per_seed~seedset,data=subset(SummaryInd,seedset>0&seedset<1), log="xy", pch=16,col=col.spp[as.factor(species)],xlab="seed set")
words.bottom.right.logxy(mod)
...
mod <- lm(asin(sqrt(seedset))~log(prop_propagule),data=subset(SummaryInd,seedset>0&seedset<1))
plot(prop_propagule~seedset,data=subset(SummaryInd,seedset>0&seedset<1), log="xy", pch=16,col=col.age[as.factor(age)],ylab="Accessory costs (asin transformed)",xlab="seed set")
words.bottom.right.logxy(mod)
...
```{r, echo=FALSE,message=FALSE, ,fig.width=14, fig.height=7}
par(mfrow=c(1,2), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.1,1.1,.1,0.2))  
mod <- lm(asin(sqrt(prop_prepollen_all))~log((ReproInv)),data=subset(SummaryInd,prop_prepollen_all>0&prop_prepollen_all<1))
plot(asin(sqrt(prop_prepollen_all))~ReproInv,data=subset(SummaryInd,prop_prepollen_all>0&prop_prepollen_all<1), log="x", pch=16,col=col.spp[as.factor(species)])
words.top.right.logx(mod)
legend("bottomleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
 
mod <- lm(asin(sqrt(prop_propagule))~log((ReproInv)),data=subset(SummaryInd,prop_propagule>0))
plot(asin(sqrt(prop_propagule))~ReproInv,data=subset(SummaryInd,prop_propagule>0), log="x", pch=16,col=col.spp[as.factor(species)])
words.top.right.logx(mod)
```
...
plot(repro_all_count~RA,data=subset(SummaryInd,repro_all_count>0),col=col.spp[as.factor(species)],pch=16,log="xy")
legend("topleft",legend=names(labels.spp),col=col.spp,pch=16, cex=.8,bty="n")
words.bottom.right.logxy(mod)
...
plot(asin(sqrt(prop_prepollen_all))~asin(sqrt(RA)),data=subset(SummaryInd,RA>0&prop_prepollen_all>0), pch=16,col=col.spp[as.factor(species)])
words.top.right(mod)

mod <- lm(asin(sqrt(RA))~asin(sqrt(prop_propagule)),data=subset(SummaryInd,RA>0))
plot(asin(sqrt(prop_propagule))~asin(sqrt(RA)),data=subset(SummaryInd,RA>0&prop_propagule>0), pch=16,col=col.spp[as.factor(species)])
words.top.right(mod)
```
...
```{r, echo=FALSE,message=FALSE}
par(mfrow=c(1,1), cex=1, omi=c(.1,.1,.1,.1), mai=c(1.1,1.1,.1,0.2))  
mod <- lm(asin(sqrt(prop_maxH))~asin(sqrt(seedset)),data=subset(SummaryInd,seedset<1&seedset>0))
...
###Plots across species, with accessory costs on a per seed produced basis
```{r, echo=FALSE, message=FALSE}
mean1 <- SummarySpp$prepollen_all_per_seed_mean
se1 <- SummarySpp$prepollen_all_per_seed_se
plot(mean1~species,data=subset(SummarySpp),cex.axis=.8,pch=16,main="Investment into pre-pollination tissues per seed produced",log="y")
se_function(mean1,se1)
```{r, echo=FALSE, message=FALSE}

```{r, echo=FALSE, message=FALSE}
mean1 <- SummarySpp$accessory_per_seed_mean
se1 <- SummarySpp$accessory_per_seed_se
plot(mean1~species,data=subset(SummarySpp),cex.axis=.8,pch=16,main="Investment into all accessory tissues per seed produced",log="y")
se_function(mean1,se1)
```{r, echo=FALSE, message=FALSE}
...
plot((mean1)~species,data=subset(SummarySpp),cex.axis=.8,pch=16,main="Investment into packaging and dispersal tissues per seed produced")
se_function(mean1,se1)
```{r, echo=FALSE, message=FALSE}
